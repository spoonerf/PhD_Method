...
opt_patch_spin_up[,4:length(spin_years)]<-1

patch_map_mine[is.na(patch_map_mine)]<-0

niche_spin_up[is.na(niche_spin_up)]<-0
opt_patch_spin_up[is.na(opt_patch_spin_up)]<-0
```
...
MatList<-list(tempMat[[6]][[1]])  #varies depending on number of matrices - need to find a way to code this better - now have five matrices available so need to sort this
...
stages<-comadre$matrixClass[keep][[6]]$MatrixClassAuthor
