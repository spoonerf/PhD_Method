...
mean_var2<-merge(LPI_EU4, mean_var, by = c("lon","lat"))
...
doVar=function(sp_name) {
  ...
  clim_yr<-clim_var[clim_var$Year %in% Year, ]$trasterex
  mvar_yr<-lm(clim_yr ~ Year)
  plot(clim_yr~Year, type="l")
  abline(lm(clim_yr ~ Year), col="red")
  var_mean_yr<-mean(clim_yr)
  varch_yr<-coef(mvar_yr)[2]
  varid<-cbind(ID,var_mean, varch, var_mean_yr, varch_yr)
  print(varid)
  return(varid)
}
...
colnames(var_df) <- c("ID", "Mean_ Var","Var_Change", "Mean_Var_Yr", "Var_Change_Yr")
