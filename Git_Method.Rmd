...
```{r}
LPI_LUC_selm<-read.csv("LPI_LandUse_121_18_09_2015.csv")
...
library(plyr)
coefs <- ddply(Climate_Temp, .(Class), function(df) {
...
q<-lm(Climate_Temp$TL ~ Climate_Temp$mean_slope_se)

library(ggplot2)
p<-ggplot(Climate_Temp,aes(x=log10(mean_slope_se),y=TL,color=Class)) +geom_point(size=3) 
