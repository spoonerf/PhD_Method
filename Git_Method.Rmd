...
for (i in 1:R) {
  ...
  sp_dups_df2<-sp_df_scale[sp_df_scale$ID %in% dt2$ID,]
    ...
    m1<-lmer(TL ~ change_rate_scale*mean_slope_scale+change_rate_scale*mean_var_scale+mean_slope_scale*mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F)
    
#     m1a<-lmer(TL ~ change_rate_scale*mean_slope_scale+change_rate_scale*mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F)
#     
#     m1b<-lmer(TL ~ change_rate_scale*mean_slope_scale+mean_slope_scale*mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F)
    
    m1c<-lmer(TL ~ +change_rate_scale*mean_var_scale+mean_slope_scale*mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F)
...
#     m2f<-lmer(TL ~mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F)
  ...
  avg.top.models.AIC<-model.avg(models_list)   
  summary(avg.top.models.AIC)

  wcoefs<-coefTable(avg.top.models.AIC, full = FALSE, adjust.se = TRUE)
...
#   marg_Rsq_m1b[i]<-modelsRsq[3,4]
  marg_Rsq_m1c[i]<-modelsRsq[2,4] #should be 4 without all other models '#'d out
