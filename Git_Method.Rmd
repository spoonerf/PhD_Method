...
LPI_EDScores<-read.csv("2016_01_08_LMEModel_data.csv")

Inc5MammalsTemp <- subset(LPI_EDScores, !is.na(SD_change_pop_yr)&!is.na(mean_slope)&r_sq2 >= 0.5 & !is.na(ED_score) & !is.na(change_rate) &Include5 == "Yes" & PA_recoded != "Unknown"& Class != "Amphibia" )
...
parm_df<-sp_dups_df[,c(6,8,11,12,13)]  ##ID, land use, and climate
parm_bird<-sp_dups_bird[,c(6,8,11,12,13)]
parm_mamm<-sp_dups_mamm[,c(6,8,11,12,13)]

parm_mat<-as.matrix(parm_df)
parm_bird_mat<-as.matrix(parm_bird)
parm_mamm_mat<-as.matrix(parm_mamm)

parm_scale<-scale(parm_mat[,c(2:5)])       #use the scaling factors at the bottom of these to scale the rasters
parm_bird_scale<-scale(parm_bird_mat[,c(2:5)])
parm_mamm_scale<-scale(parm_mamm_mat[,c(2:5)])
...
colnames(parm_df_scale)<-c("ID", "mean_slope_scale", "mean_var_scale", "mean_rng_scale", "change_rate_scale")

colnames(parm_birddf_scale)<-c("ID", "mean_slope_scale", "mean_var_scale","mean_rng_scale", "change_rate_scale")

colnames(parm_mammdf_scale)<-c("ID", "mean_slope_scale", "mean_var_scale","mean_rng_scale", "change_rate_scale")
...
for (i in 1:R) {
  ...
  sp_dups_df2<-sp_df_scale[sp_df_scale$ID %in% dt2$ID,]
    ...
    m1<-lmer(TL ~ change_rate_scale*mean_slope_scale+change_rate_scale*mean_rng_scale+mean_slope_scale*mean_rng_scale+(1|Binomial_old)+(1|Country),data=sp_dups_df2)
  
    m2<-lmer(TL ~change_rate_scale+mean_slope_scale+mean_rng_scale+(1|Binomial_old)+(1|Country),data=sp_dups_df2)
