...
LPI_EDScores3<-merge(LPI_EDScores3, hot_df, by="ID")
...
Inc5MammalsTemp <- subset(LPI_EDScores3, !is.na(SE)&!is.na(Estimate)&r_sq2 >= 0.5  & !is.na(change_rate) &Include10 == "Yes" & System != "Marine")
...
parm_df<-sp_dups_df[,c("ID","Estimate", "hot_avg", "change_rate_25")]  ##ID, land use, and climate
...
parm_scale<-scale(parm_mat[,c("Estimate", "hot_avg", "change_rate_25")])       #use the scaling factors at the bottom of these to scale the rasters
...
change_mean_se2 = numeric(R)
mean_se_mean_slope2 = numeric(R)
interceptw = numeric(R)
change_ratew = numeric(R)
mean_sew = numeric(R)
mean_slopew = numeric(R)
change_mean_sew = numeric(R)
change_mean_slopew = numeric(R)
mean_se_mean_slopew = numeric(R)
...
#   msAICc <- model.sel(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2f)
...
lowCI_av<-c(sort(int_av)[25],sort(LUC_av)[2],sort(MTV_av)[2],sort(MTC_av)[2],sort(LUC_MTV_av)[2],sort(LUC_MTC_av)[2],sort(MTC_MTV_av)[2])

highCI_av<-c(sort(int_av)[97],sort(LUC_av)[97],sort(MTV_av)[97],sort(MTC_av)[97],sort(LUC_MTV_av)[97],sort(LUC_MTC_av)[97],sort(MTC_MTV_av)[97])
