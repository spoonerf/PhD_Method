...
#LPI_EU5<-LPI_EU4[,c(1,2,3,30,31,64:126)]
...
doVar=function(sp_name) {
  sp_var=subset(mean_var3, id==sp_name)
  ID<-sp_var$id[1]
  clim_var<-aggregate(trasterex ~  Year , sp_var , sd )
  clim_range<-aggregate(trasterex ~  Year , sp_var , range )
  # clim_var$Year<-as.numeric(clim_var$Year)
  # mvar<-lm(clim_var$trasterex ~ clim_var$Year)
  # plot(clim_var$trasterex~clim_var$Year, type="l")
  # var_mean<-mean(clim_var$trasterex)
  # varch<-coef(mvar)[2]
  # varid<-cbind(ID,var_mean, varch)
  varid<-cbind(ID, clim_var, clim_range)
  print(varid)
  return(varid)
}

all_var<-lapply(unique(mean_var3$id), doVar)

all_matrix <- matrix(unlist(all_var), ncol=6, byrow=F)

var_df <- data.frame(all_matrix)
colnames(var_df) <- c("ID","Year", "Mean_TEmp_SD", "Year", "Range_min", "Range_max")

var_df$range<-var_df$Range_max - var_df$Range_min


LPI_EU5<-LPI_EU4[,c(1,2,3,30,31,64:126)]

mean_var4<-merge(LPI_EU5, var_df, by = "ID")


doVarch=function(sp_name) {
  spid2 = subset(mean_var4, ID == sp_name)   #subsetting the population data by each population
  spid = spid2[,64:126]                     #subsetting only the dates
  colnames(spid)<-1950:2012
