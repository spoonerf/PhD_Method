...
LPI_EDScores3<-merge(LPI_EDScores3, sens, by="ID")
...
parm_df<-sp_dups_df[,c("ID","mean_slope", "mean_slope_se", "change_rate_9")]  ##ID, land use, and climate
...
parm_scale<-scale(parm_mat[,c("mean_slope", "mean_slope_se", "change_rate_9")])       #use the scaling factors at the bottom of these to scale the rasters
 ...
 #  n2d<-lmer(TL ~change_rate_scale+(1|Binomial_old),data=sp_dups_df2)
  ...
  #  n2f<-lmer(TL ~mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F
    ...
    mnull<-lmer(TL ~ (1|Binomial_old),data=sp_dups_df2, REML=F)
      ...
      models_list<-list(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2f,m3,m4,mnull)
  #models_list<-list(m1,m1c)

  modelsR<-lapply(models_list,rsquared.glmm)
  modelsRsq <- matrix(unlist(modelsR), ncol=6, byrow=T)

  msAICc <- model.sel(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2f,m3,m4,mnull)
  ...
  marg_Rsq_mnull[i]<-modelsRsq[17,4]
  ...
  cond_Rsq_mnull[i]<-modelsRsq[17,5]
  ...
  BM_av[i]<-var_imp$coefmat.subset["Bodymass"]
  
  # intercept2[i]<-coef(summary(m1c))[ 1, "Estimate"]
  # change_rate2[i]<-coef(summary(m1c))[ 2, "Estimate"]
  # mean_se2[i]<-coef(summary(m1c))[ 3, "Estimate"]
  # mean_slope2[i]<-coef(summary(m1c))[4, "Estimate"]
  # change_mean_se2[i]<-coef(summary(m1))[5, "Estimate"]
  # mean_se_mean_slope2[i]<-coef(summary(m1))[6, "Estimate"]
  # 
  # interceptw[i]<-wcoefs[1, "Estimate"]
  # change_ratew[i]<-wcoefs[2, "Estimate"]
  # mean_sew[i]<-wcoefs[3, "Estimate"]
  # mean_slopew[i]<-wcoefs[4, "Estimate"]
  # change_mean_sew[i]<-wcoefs[5, "Estimate"]
  # change_mean_slopew[i]<-wcoefs[6, "Estimate"]
  # mean_se_mean_slopew[i]<-wcoefs[6, "Estimate"]
  # 
  print(i)
}
