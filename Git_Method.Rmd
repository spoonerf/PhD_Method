...
For each of the 1196 remaining populations the time series of population data was modelled using either a General Additive Model or a Linear Model. As in Collen 2009 - populations with less than six recorded data points in the time series were modelled using a linear model. Those with more than six data points were modelled using a General Additive Model. This was done so that missing data (for years when the population wasn't estimated) could be interpolated from the fitted models.
...
doFit = function(sp_name) {
  ...
  if (points >=6) {           ###should I be trying GAMs for populations with less than six points and if that doesn't fit then use a linear model - here I am automatically fitting a linear model if there are less than six points
    PopN = log10(Population)   #log and log10
    ...
    mg2<-mgcv:::gam(Population ~ s(Year, k=SmoothParm), fx=TRUE)
    pv2<- predict(mg2,df,type="response",se=TRUE) 
    R_sq2<-summary(mg)$r.sq
    model<-1
    #pv$fit[pv$fit <= 0] <- NA
...
all_df_list <- lapply(unique(LPI_EU4$ID), doFit)
...
write.csv(all_df, "Europe_Population_Trends_Rsq_Lambda_16_03_03.csv")
write.csv(all_df, "Europe_Population_Trends_Rsq_Lambda_16_03_03a.csv")
...
Pop_tr<-read.csv("Europe_Population_Trends_Rsq_Lambda_16_03_02.csv")
Pop_tr<-read.csv("Europe_Population_Trends_Rsq_Lambda_16_03_03.csv")
#pop2<-read.csv("Europe_Population_Trends_Rsq_Lambda_16_03_03.csv")

chck<-Pop_tr[Pop_tr$ID %in% LPI_EDScores$ID,]

chckm<-merge(chck, LPI_EDScores, by="ID")

chckm$lambda_sum<-round(chckm$lambda_sum, digits=3)
chckm$TL<-round(chckm$TL, digits=3)

chckm$ldiff<-chckm$lambda_sum - chckm$TL

# chckm$lambda_sum2.x<-round(chckm$lambda_sum2.x, digits=3)
# chckm$TL<-round(chckm$TL, digits=3)
# 
# chckm$ldiff2<-chckm$lambda_sum2.x - chckm$TL
# 

subset(chckm, ldiff >1.5 & r_sq >0.5| ldiff < -1.5 & r_sq >0.5)

subset(chckm, ldiff == 0)$model.x
...
LPI_EDScores2<-merge(LPI_EDScores2, res_prec, by="ID", all=T)
...
Inc5MammalsTemp <- subset(LPI2, !is.na(MnSlope50k)&r_sq >= 0.5  & !is.na(change_rate_49)&length_time >= 30 & System != "Marine")
...
#Inc5MammalsTemp <- subset(LPI_EDScores2, !is.na(change_rate_25)&!is.na(mean_slope) &System != "Marine"&length_time >= 5)

#Inc5MammalsTemp <- subset(LPI_EDScores, !is.na(mean_slope_se)&!is.na(mean_slope)&r_sq2 >= 0.5  & !is.na(change_rate) &Include10 == "Yes" & System != "Marine")

#Inc5MammalsTemp <- subset(LPI_EDScores3, !is.na(mean_slope_se)&!is.na(mean_slope)&r_sq2 >= 0.5  & !is.na(change_rate) &Include10 == "Yes" & System != "Marine")
# 
# Inc5MammalsTemp <- subset(LPI_EDScores2, !is.na(Estimate)&r_sq2 >= 0.5  & !is.na(change_rate_25) &length_time >= 5 & System != "Marine")
# 
# Inc5MammalsTemp <- subset(LPI_EDScores2, !is.na(mean_slope)&r_sq2 >= 0.5  & !is.na(change_rate_49)&length_time >= 5  & System != "Marine")
# 
...
parm_df<-sp_dups_df[,c("ID","MnSlope50k", "mean_range_var",  "change_rate_49")]  ##ID, land use, and climate

parm_mat<-as.matrix(parm_df)
parm_scale<-scale(parm_mat[,c("MnSlope50k", "mean_range_var", "change_rate_49")])       #use the scaling factors at the bottom of these to scale the rasters
...
#sp_df_scale<-subset(sp_df_scale, Class=="Mammalia")
sp_df_scale<-subset(sp_df_scale, Class=="Aves")
...
for (i in 1:R) {
  ...
  # n1f<-lmer(lambda_sum ~ mean_slope_scale*mean_var_scale+(1|Binomial),data=sp_dups_df2)
   ...
   # m1g<-lmer(lambda_sum ~ change_rate_scale*prec_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
 ...
 #  n2d<-lmer(lambda_sum ~change_rate_scale+(1|Binomial),data=sp_dups_df2)
  ...
  #  n2e<-lmer(lambda_sum ~mean_slope_scale+(1|Binomial),data=sp_dups_df2)
    ...
    #m2g<-lmer(lambda_sum ~ prec_var_scale+ (1|Binomial), data=sp_dups_df2, REML=F)
    
  #  m3<-lmer(lambda_sum ~mean_slope_scale+Bodymass+(1|Binomial),data=sp_dups_df2, REML=F)
   # n2f<-lmer(lambda_sum ~mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  ...
  msAICc <- model.sel(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2f,mnull)
...
#  marg_Rsq_m3[i]<-modelsRsq[15,4]
   marg_Rsq_mnull[i]<-modelsRsq[15,4]
  ...
  cond_Rsq_mnull[i]<-modelsRsq[15,5]
...
AIC_df<-data.frame(cbind(AIC_m1,AIC_m1a, AIC_m1b, AIC_m1c, AIC_m1d, AIC_m1e, AIC_m1f, AIC_m2,AIC_m2a, AIC_m2b, AIC_m2c, AIC_m2d, AIC_m2e, AIC_m2f,AIC_mnull))


AIC_del<-AIC_df[,c(1:14)] - AIC_df$AIC_mnull
