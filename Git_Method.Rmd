 ...
 #  n2d<-lmer(TL ~change_rate_scale+(1|Binomial_old),data=sp_dups_df2)
  ...
  #  n2f<-lmer(TL ~mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F
    ...
    mnull<-lmer(TL ~ (1|Binomial_old),data=sp_dups_df2, REML=F)
      ...
      models_list<-list(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2f,m3,m4,m5,mnull)
  #models_list<-list(m1,m1c)

  modelsR<-lapply(models_list,rsquared.glmm)
  modelsRsq <- matrix(unlist(modelsR), ncol=6, byrow=T)

  msAICc <- model.sel(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2f,m3,m4,m5,mnull)
  ...
  marg_Rsq_mnull[i]<-modelsRsq[18,4]
  ...
  cond_Rsq_mnull[i]<-modelsRsq[18,5]
...
mean_av<-c(mean(int_av), mean(LUC_av), mean(MTV_av),mean(MTC_av), mean(LUC_MTV_av), mean(LUC_MTC_av), mean(MTC_MTV_av), mean(BM_av))
 
lowCI_av<-c(sort(int_av)[250],sort(LUC_av)[250],sort(MTV_av)[250],sort(MTC_av)[250],sort(LUC_MTV_av)[250],sort(LUC_MTC_av)[250],sort(MTC_MTV_av)[250], sort(BM_av)[250])

highCI_av<-c(sort(int_av)[9750],sort(LUC_av)[9750],sort(MTV_av)[9750],sort(MTC_av)[9750],sort(LUC_MTV_av)[9750],sort(LUC_MTC_av)[9750],sort(MTC_MTV_av)[9750],sort(BM_av)[9750])

Variable<-c( "Intercept", "LUC", "MTV", "MTC", "LUC*MTV", "LUC*MTC", "MTV*MTC", "BM")
...
# par(mar=c(14,5,1,1))
# par(mgp=c(2, 1,0))
plotCI(1:8, mean_av, (highCI_av-mean_av), (mean_av-lowCI_av), ylab="Coefficient Values", xlab="" ,xaxt = "n", main="Model Averaged Coefficients", lwd=1)
axis(1, at=1:8, labels=colnames(conf_av), las=2)
