...
df<-data.frame(Year,PopN)  
plot(Year,PopN, type="o", ylab=expression("Log"[10]* " Population"), main=expression(Alpine~ibex~italic((Capra~ibex))), ylim=c(min(na.omit(PopN*0.975)),max(na.omit(PopN*1.025))))
...
ggplot(data=df,
       aes(x=Year, y=PopN)) +
      geom_line()+
       geom_line( y=PopN*0.975, colour="red")+
        geom_line( y=PopN*1.025, colour="red")+
      theme_bw()
```
...
```{r}
Inc5MammalsTemp <- subset(LPI_EDScores2, !is.na(MnSlope50k)&r_sq >= 0.5  & !is.na(change_rate_49)&length_time >=5 & !is.na(Bodymass) &System!="Marine" )
...
#write.csv(sp_df_scale, "Europe_HILDA_5_year_pops.csv")
