...
Inc5MammalsTemp <- subset(LPI_EDScores, !is.na(SD_change_pop_yr)&!is.na(mean_slope)&r_sq2 >= 0.5 & !is.na(ED_score) & !is.na(change_rate) &Include5 == "Yes" & PA_recoded != "Unknown"& Class != "Amphibia" &length_time >=10)
...
parm_df<-sp_dups_df[,c(6,8:13)]  ##ID, land use, and climate
parm_bird<-sp_dups_bird[,c(6,8:13)]
parm_mamm<-sp_dups_mamm[,c(6,8:13)]

parm_mat<-as.matrix(parm_df)
parm_bird_mat<-as.matrix(parm_bird)
parm_mamm_mat<-as.matrix(parm_mamm)

parm_scale<-scale(parm_mat[,c(2:7)])       #use the scaling factors at the bottom of these to scale the rasters
parm_bird_scale<-scale(parm_bird_mat[,c(2:7)])
parm_mamm_scale<-scale(parm_mamm_mat[,c(2:7)])
