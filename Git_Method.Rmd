...
# xy<-matrix(c(7,52), nrow=1, ncol=2)
# xy<-SpatialPoints(xy)

#xyex<- extract(r2[[1:23741]], xy, buffer=50000, fun=mean, na.rm=TRUE)

mean_var<-data.frame(id=numeric(0), Binomial=character(0),xy=numeric(0), Year=numeric(0), Month=numeric(0), rasterex=numeric(0))

for (i in 1:nlayers(r2)) {
  
  rasterex <- extract(r2[[i]], xy, buffer=50000, fun=mean, na.rm=TRUE)
  Date<-as.Date(substr(names(r2[[i]]), 2, 11), "%Y.%m.%d")
  Year<-format(Date,format="%Y")
  Month<-months(Date)
  dataex<-data.frame(id, binomial,xy, Year,Month, rasterex)   
  print(Date)
