...
LPI_EDScores3<-merge(LPI_EDScores3, sens, by="ID")    #check this
LPI_EDScores3<-merge(LPI_EDScores3, clim_sens, by="ID")
...
Inc5MammalsTemp <- subset(LPI_EDScores3, !is.na(change_rate_25)&!is.na(mean_slope) &System != "Marine"&Include10 == "Yes")
...
#Inc5MammalsTemp <- subset(LPI_EDScores3, !is.na(SE25k)&!is.na(Estimate25k)&r_sq2 >= 0.5  & !is.na(change_rate) &Include10 == "Yes" & System != "Marine")

nrow(Inc5MammalsTemp)


hist(Inc5MammalsTemp$mean_slope_se, breaks=100, xlab=expression(Residual~Variation~"in"~Linear~Regression~of~Mean~Temperature(~degree~C)), main="")
...
parm_df<-sp_dups_df[,c("ID","mean_slope", "SD_change_pop_yr", "change_rate_25")]  ##ID, land use, and climate
#parm_df<-sp_dups_df[,c("ID","SD_change_all_yr","Range_change_all_yr","SD_change_pop_yr","Range_change_pop_yr", "max_slope","max_slope_se","max_mon_var","max_year_var","min_slope","min_slope_se","min_mon_var","min_year_var","pcp_slope","pcp_slope_se","pcp_mon_var","pcp_year_var","mean_slope","mean_slope_se","mean_mon_var","mean_year_var","change_rate")]
...
parm_scale<-scale(parm_mat[,c("mean_slope", "SD_change_pop_yr", "change_rate_25")])       #use the scaling factors at the bottom of these to scale the rasters
#parm_scale<-scale(parm_mat[,c("ID","SD_change_all_yr","Range_change_all_yr","SD_change_pop_yr","Range_change_pop_yr", "max_slope","max_slope_se","max_mon_var","max_year_var","min_slope","min_slope_se","min_mon_var","min_year_var","pcp_slope","pcp_slope_se","pcp_mon_var","pcp_year_var","mean_slope","mean_slope_se","mean_mon_var","mean_year_var","change_rate")])
...
#colnames(parm_df_scale)<-c("ID","SD_change_all_yr_scale","Range_change_all_yr_scale","SD_change_pop_yr_scale","Range_change_pop_yr_scale", "max_slope_scale","max_slope_se_scale","max_mon_var_scale","max_year_var_scale","min_slope_scale","min_slope_se_scale","min_mon_var_scale","min_year_var_scale","pcp_slope_scale","pcp_slope_se_scale","pcp_mon_var_scale","pcp_year_var_scale","mean_slope_scale","mean_slope_se_scale","mean_mon_var_scale","mean_year_var_scale","change_rate_scale")
...
#d<-lmer(TL ~ Range_change_pop_yr_scale+max_slope_scale+max_slope_se_scale+min_slope_scale+min_slope_se_scale+pcp_slope_scale+pcp_slope_se_scale+mean_slope_scale+mean_slope_se_scale+change_rate_scale+(1|Binomial_old)+(1|Country),data=sp_df_scale)
...
for (i in 1:R) {
 ...
 #  n2d<-lmer(TL ~change_rate_scale+(1|Binomial_old),data=sp_dups_df2)
  ...
  #  n2f<-lmer(TL ~mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F
    ...
    #m3<-lmer(TL ~ Bodymass + (1|Binomial_old),data=sp_dups_df2, REML=F)
    
    #m4<- lmer(TL ~ Bodymass + change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_slope_scale+change_rate_scale:mean_var_scale+mean_slope_scale:mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F)
    
    #m5<-lmer(TL ~ Bodymass + change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_var_scale+mean_slope_scale:mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F)
