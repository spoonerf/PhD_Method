...
LPI_EDScores2<-merge(LPI_EDScores2, res_mx, by="ID")
...
parm_df<-sp_dups_df[,c("ID","Estimate50k", "range_var", "change_rate_49")]  ##ID, land use, and climate

parm_mat<-as.matrix(parm_df)
parm_scale<-scale(parm_mat[,c("Estimate50k", "range_var", "change_rate_49")])       #use the scaling factors at the bottom of these to scale the rasters
  ...
  #  n2e<-lmer(TL ~mean_slope_scale+(1|Binomial_old),data=sp_dups_df2)
   ...
   # n2f<-lmer(TL ~mean_var_scale+(1|Binomial_old),data=sp_dups_df2, REML=F)
    ...
    mnull<-lmer(TL ~(1|Binomial_old),data=sp_dups_df2, REML=F)
      ...
      models_list<-list(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2f,m3,mnull)
  modelsR<-lapply(models_list,rsquared.glmm)
  modelsRsq <- matrix(unlist(modelsR), ncol=6, byrow=T)

  msAICc <- model.sel(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2f,m3)
...
AIC_df<-data.frame(cbind(AIC_m1,AIC_m1a, AIC_m1b, AIC_m1c, AIC_m1d, AIC_m1e, AIC_m1f, AIC_m2,AIC_m2a, AIC_m2b, AIC_m2c, AIC_m2d, AIC_m2e, AIC_m2f, AIC_m3, AIC_mnull))


AIC_del<-AIC_df[,c(1:15)] - AIC_df$AIC_mnull
