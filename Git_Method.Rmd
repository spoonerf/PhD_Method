...
parm_df<-sp_dups_df[,c(6,8,11,13)]  ##ID, land use, and climate
parm_bird<-sp_dups_bird[,c(6,8,11,13)]
parm_mamm<-sp_dups_mamm[,c(6,8,11,13)]
...
for (i in 1:R) {
  ...
  sp_dups_df2<-sp_df_scale[sp_df_scale$ID %in% dt2$ID,]
    ...
    m1<-lmer(TL ~ change_rate_scale*mean_slope_scale+(1|Binomial_old)+(1|Country),data=sp_dups_df2)
  ...
  mean_slope[i]<-coef(summary(m1))[ 3, "Estimate"]
  #change_mean_se[i]<-coef(summary(m1))[ 5, "Estimate"]
  change_mean_slope[i]<-coef(summary(m1))[ 4, "Estimate"]
