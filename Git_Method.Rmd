...
doFit = function(sp_name) {
  ...
  Population[Population == 0] <- mean(Population, na.rm=TRUE)*0.01    #replacing zeros with 1% of the mean population as you cannot log0
  ...
  if (points >=6) {           ###should I be trying GAMs for populations with less than six points and if that doesn't fit then use a linear model - here I am automatically fitting a linear model if there are less than six points
    ...
    mg<-mgcv:::gam(PopN ~ s(Year, k=SmoothParm), fx=TRUE)
    pv <- predict(mg,df,type="response",se=TRUE) 
    R_sq<-summary(mg)$r.sq
    model<-1
    pv$fit[pv$fit <= 0] <- NA
    lambda<-diff(pv$fit)
    ...
    PopN = log10(Population)
    ml<-lm(PopN~df$Year)
    R_sq<-summary(ml)$r.sq
    model<-0
    Pop_interp<-na.approx(PopN)
    Pop_interp[Pop_interp<=0] <- NA
    lambda<-diff(Pop_interp)
    lambda_sum<-sum(lambda, na.rm=TRUE)
    lambda_mean<-mean(lambda, na.rm=TRUE)
  }

  res_df = data.frame(sp_name=sp_name, points=points, SmoothParm=SmoothParm, r_sq=R_sq, model=model,lambda_sum=lambda_sum,lambda_mean=lambda_mean,time=time, missing=missing)
...
write.csv(all_df, "Global_Population_Trends_Rsq_Lambda_16_03_16.csv")
...
```{r Land Use Change, message=FALSE, warning=FALSE, Land Use Change, eval=FALSE}
...
bird<-read.csv("2014 BirdLife list with parameters for LPI analysis no head.csv")

head(bird)

colnames(bird)

bird_bm<-bird[,c(1,3,40)]

LPI<-read.csv("LPI_populations_IP_fishedit_20140310_nonconf.csv")
LPI_bird<-subset(LPI, Class=="Aves")
bird_id<-LPI_bird[,c(1,2,20)]

bird_match<-bird_id[bird_id$Binomial %in% bird_bm$Scientific_name,]
bird_nomatch<-bird_id[!(bird_id$Binomial %in% bird_bm$Scientific_name),]

match_merge<-merge(bird_match, bird_bm, by.x="Binomial", by.y="Scientific_name")

bird_com_match<-bird_nomatch[bird_nomatch$Common_name %in% bird_bm$Common.name, ]

match_merge2<-merge(bird_com_match, bird_bm, by.x="Binomial", by.y="Scientific_name")  #no overlapping species

bird_com_nomatch<-bird_nomatch[!(bird_nomatch$Common_name %in% bird_bm$Common.name), ]

bird_2match<-bird_nomatch[bird_nomatch$Binomial %in% Inc5MammalsTemp$Binomial,]   #checking which ones I actually need - using Inc5MammalsTemp from later chunk

bird_2match$Binomial<-as.character(bird_2match$Binomial)
#manually correcting errors
bird_2match$Binomial[bird_2match$Binomial == "Aquila_fasciatus"] <- "Aquila_fasciata"
bird_2match$Binomial[bird_2match$Binomial == "Philomachus_pugnax"] <- "Calidris_pugnax"
bird_2match$Binomial[bird_2match$Binomial == "Anas_clypeata"] <- "Spatula_clypeata"
bird_2match$Binomial[bird_2match$Binomial == "Anas_strepera"] <- "Mareca_strepera"
bird_2match$Binomial[bird_2match$Binomial == "Sterna_nilotica"]<-"Gelochelidon_nilotica"
bird_2match$Binomial[bird_2match$Binomial == "Sterna_albifrons"]<-"Sternula_albifrons"
bird_2match$Binomial[bird_2match$Binomial == "Casmerodius_albus"]<-"Ardea_alba"
bird_2match$Binomial[bird_2match$Binomial ==  "Porzana_parva"] <-"Zapornia_parva"


match_merge3<-merge(bird_2match, bird_bm, by.x="Binomial", by.y="Scientific_name")

all_merge<-rbind(match_merge, match_merge2, match_merge3)

all_merge<-all_merge[,c(1,2,5)]
colnames(all_merge)<-c("Binomial_BL", "ID", "Bodymass_g")

all_merge$Bodymass<-log10(all_merge$Bodymass_g)

write.csv(all_merge, "Birdlife_Bodymass.csv")

```

```{r, mammal traits}
mm<-read.csv("Mammals_matchup.csv")
pan<-read.csv("Pantheria_update_8.6.05.csv")
mm<-mm[,c(1,5)]
pan<-pan[,c(3,10)]
...
#pan2<-pan[pan$MSW93_Binomial%in% mm$PanTHERIA,]   #three not in pan

mm2<-merge(mm, pan, by.x="PanTHERIA", by.y="MSW93_Binomial", all=T)

LPI<-read.csv("LPI_populations_IP_fishedit_20140310_nonconf.csv")
LPI_mamm<-subset(LPI, Class=="Mammalia")
mamm_id<-LPI_mamm[,c(1,2,20)]

mm3<-merge(mm2, mamm_id, by.x="LPI_binomial", by.y="Binomial")


mm3$LPI_binomial<-as.character(mm3$LPI_binomial)
mm3$PanTHERIA<-as.character(mm3$PanTHERIA)
mm3$LPI_binomial == mm3$PanTHERIA

mm3$Bodymass<-log10(mm3$AdultBodyMass_g)

mm3<-mm3[,c(1,4,3,6)]

write.csv(mm3, "PanTHERIA_bodymass.csv")

```

```{r, herp traits}
rep<-read.csv("feldman_et_al_reptile_traits.csv")

LPI<-read.csv("LPI_populations_IP_fishedit_20140310_nonconf.csv")
LPI_herp<-subset(LPI, Class=="Amphibia"| Class=="Reptilia")
herp_id<-LPI_herp[,c(1,2,20)]

rep$binomial<-gsub(" ", "_", rep$binomial)

hmrg<-merge(herp_id, rep, by.x="Binomial", by.y="binomial")

herp_id[herp_id$Binomial %in% hmrg$Binomial,]

herp_id[!(herp_id$Binomial %in% hmrg$Binomial),]

head(hmrg)

hmrg<-hmrg[,c(1,2,14,13)]

write.csv(hmrg, "reptile_bodymass.csv")
...
LPI<-read.csv("LPI_populations_IP_fishedit_20140310_nonconf.csv")
LPI_amph<-subset(LPI, Class=="Amphibia")
amph_id<-LPI_amph[,c(1,2,20)]
amph<-read.csv("European_Amphibians_Database3.csv")

amph$Species<-gsub(" ", "_", amph$Species)

amph<-amph[,c(1,5)]

amphmg<-merge(amph, amph_id, by.x="Species", by.y="Binomial")
amphmg$Adult.body.mass<-as.numeric(as.character(amphmg$Adult.body.mass))
amphmg$Bodymass<-log10(amphmg$Adult.body.mass)

amphmg<-amphmg[,c(1,3,2,5)]

write.csv(amphmg, "amphibian_bodymass.csv")
```


```{r, combine traits}
colnames(all_merge)<-c("Binomial", "ID", "Bodymass_g", "Bodymass")
colnames(mm3)<-c("Binomial", "ID", "Bodymass_g", "Bodymass")
colnames(hmrg)<-c("Binomial", "ID", "Bodymass_g", "Bodymass")
colnames(amphmg)<-c("Binomial", "ID", "Bodymass_g", "Bodymass")

bmt<-rbind(all_merge, mm3, hmrg,amphmg)

write.csv(bmt, "LPI_traits.csv")
...
Climate_Temp2<- subset(Clim_pop, !is.na(mean_slope)&!is.na(Range_change_pop_yr)&r_sq2 >= 0.5 &Include5 == "Yes"&Class != "Amphibia" )
...
library(effects)
mx<-lmer(TL~mean_slope*mean_slope_se +(1|Binomial_old)+ (1|Country), data=Climate_Temp, REML=F)

plot(effect("mean_slope*mean_slope_se",xlevels=4, mx, factor.names=FALSE, layout=c(4,1), alternating=FALSE))

mx<-lmer(TL~mean_slope*SD_change_all_yr +(1|Binomial_old)+ (1|Country), data=Climate_Temp2, REML=F)

plot(effect("mean_slope*SD_change_all_yr",xlevels=4, mx, factor.names=FALSE, layout=c(4,1), alternating=FALSE))
...
coefs <- ddply(clim_dups_df, .(Class), function(df) {
  ...
  n<- summary(lm(TL ~ Range_change_pop_yr, data=df))
...
#LPI_EDScores2<-read.csv("2016_01_08_LMEModel_data.csv")   #seems to be 10 fewer populations in this one? 

#LPI_EDScores3<-merge(LPI_EDScores2[,c(2,5:8)], LPI_EDScores, by="ID")

#LPI_EDScores<-LPI_EDScores[,c(2,16,20,22,25,27:34,47:75,223,245:247)]

LPI<-read.csv("LPI_populations_IP_fishedit_20140310_nonconf.csv")

LPI2<-LPI[,c("ID","Binomial","Common_name", "System", "Class", "Longitude", "Latitude")]
#LU_sens<-read.csv("Sensitivity_Land_Use.csv")
LU_sens<-read.csv("Sensitivity_Land_Use_morepop.csv")
Pop_tr<-read.csv("Europe_Population_Trends_Rsq_Lambda_16_03_03.csv")
clim_sens<-read.csv("Climate_Sensitivity_2016_03_02.csv")
bm<-read.csv("LPI_traits.csv")
res_mean<-read.csv("mean_temp_residuals_all_EU.csv")
#res_prec<-read.csv("prec_residuals_all_EU.csv")

res_mean<-res_mean[,c(2,7)]
colnames(res_mean)<-c("ID","mean_res_quart")
#res_prec<-res_prec[,c(2,8,9)]
#colnames(res_prec)<-c("ID","prec_var_res", "prec_range_var")

LPI_EDScores2<-merge(LPI2, LU_sens, by="ID",all=T)
LPI_EDScores2<-merge(LPI_EDScores2, clim_sens, by="ID",all=T)
LPI_EDScores2<-merge(LPI_EDScores2, res_mean, by="ID",all=T)
LPI_EDScores2<-merge(LPI_EDScores2, bm[,c(3,5)], by="ID",all=T)
LPI_EDScores2<-merge(LPI_EDScores2, Pop_tr, by="ID",all=T)

#########
# df<-merge(merge(Pop_tr, LU_sens,by="ID", all=T), merge(clim_sens, res_mean, by="ID", all=T), by="ID", all=T)
# 
# LPI<-read.csv("LPI_populations_IP_fishedit_20140310_nonconf.csv")
# 
# LPI2<-merge(df, LPI[,c(1,2,13,20,26,29,30,33)])
# 
# Inc5MammalsTemp <- subset(LPI2, !is.na(MnSlope50k)&r_sq >= 0.5  & !is.na(change_rate_49)&length_time >= 5 & System != "Marine")
# 
...
```{r}
Inc5MammalsTemp <- subset(LPI_EDScores2, !is.na(MnSlope50k)&r_sq >= 0.5  & !is.na(change_rate_49)&length_time >=35 & !is.na(Bodymass) &System!="Marine" & Class=="Mammalia")
...
marg_Rsq_m1d = numeric(R)
cond_Rsq_m1d = numeric(R)
marg_Rsq_m1e = numeric(R)
cond_Rsq_m1e = numeric(R)
marg_Rsq_m1f = numeric(R)
cond_Rsq_m1f = numeric(R)

marg_Rsq_m2 = numeric(R)
cond_Rsq_m2 = numeric(R)
marg_Rsq_m2a = numeric(R)
cond_Rsq_m2a = numeric(R)
marg_Rsq_m2b = numeric(R)
cond_Rsq_m2b = numeric(R)
marg_Rsq_m2c = numeric(R)
cond_Rsq_m2c = numeric(R)
marg_Rsq_m2d = numeric(R)
cond_Rsq_m2d = numeric(R)
marg_Rsq_m2e = numeric(R)
cond_Rsq_m2e = numeric(R)
marg_Rsq_m2f = numeric(R)
cond_Rsq_m2f = numeric(R)
marg_Rsq_mnull = numeric(R)
...
LUC_av = numeric(R)
MTV_av = numeric(R)
MTC_MTV_av = numeric(R)
LUC_MTV_av = numeric(R)
...
for (i in 1:R) {
  ...
  sp_dups_df2<-sp_df_scale[sp_df_scale$ID %in% dt2$ID,]
    ...
    m1<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_slope_scale+change_rate_scale:mean_var_scale+mean_slope_scale:mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  #n1<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_slope_scale+change_rate_scale:mean_var_scale+mean_slope_scale:mean_var_scale+(1|Binomial),data=sp_dups_df2)
    
    m1a<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_slope_scale+change_rate_scale:mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  # n1a<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_slope_scale+change_rate_scale:mean_var_scale+(1|Binomial),data=sp_dups_df2)
    
    m1b<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_slope_scale+mean_slope_scale:mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
   #n1b<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_slope_scale+mean_slope_scale:mean_var_scale+(1|Binomial),data=sp_dups_df2)

    m1c<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_var_scale+mean_slope_scale:mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
   #n1c<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+mean_var_scale+change_rate_scale:mean_var_scale+mean_slope_scale:mean_var_scale+(1|Binomial),data=sp_dups_df2)
    
    m1d<-lmer(lambda_mean ~ change_rate_scale*mean_slope_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  # n1d<-lmer(lambda_mean ~ change_rate_scale*mean_slope_scale+(1|Binomial),data=sp_dups_df2)
  
    m1e<-lmer(lambda_mean ~ change_rate_scale*mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  # n1e<-lmer(lambda_mean ~ change_rate_scale*mean_var_scale+(1|Binomial),data=sp_dups_df2)
    
    m1f<-lmer(lambda_mean ~ mean_slope_scale*mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  # n1f<-lmer(lambda_mean ~ mean_slope_scale*mean_var_scale+(1|Binomial),data=sp_dups_df2)
    
    m2<-lmer(lambda_mean ~change_rate_scale+mean_slope_scale+mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  #n2<-lmer(lambda_mean ~change_rate_scale+mean_slope_scale+mean_var_scale+(1|Binomial),data=sp_dups_df2)
    
    m2a<-lmer(lambda_mean ~change_rate_scale+mean_slope_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  #  n2a<-lmer(lambda_mean ~change_rate_scale+mean_slope_scale+(1|Binomial),data=sp_dups_df2)
    
    m2b<-lmer(lambda_mean ~change_rate_scale+mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
 #  n2b<-lmer(lambda_mean ~change_rate_scale+mean_var_scale+(1|Binomial),data=sp_dups_df2)
    
    m2c<-lmer(lambda_mean ~mean_slope_scale+mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  # n2c<-lmer(lambda_mean ~mean_slope_scale+mean_var_scale+(1|Binomial),data=sp_dups_df2)
    
    m2d<-lmer(lambda_mean ~change_rate_scale+(1|Binomial),data=sp_dups_df2, REML=F)
 #  n2d<-lmer(lambda_mean ~change_rate_scale+(1|Binomial),data=sp_dups_df2)
    
    m2e<-lmer(lambda_mean ~mean_slope_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  #  n2e<-lmer(lambda_mean ~mean_slope_scale+(1|Binomial),data=sp_dups_df2)
    
    m2f<-lmer(lambda_mean ~mean_var_scale+(1|Binomial),data=sp_dups_df2, REML=F)

    m3<-lmer(lambda_mean ~ Bodymass +(1|Binomial),data=sp_dups_df2, REML=F)
    
    m4<-lmer(lambda_mean ~ Bodymass+mean_var_scale+mean_slope_scale+change_rate_scale +(1|Binomial),data=sp_dups_df2, REML=F)
    
    mnull<-lmer(lambda_mean ~(1|Binomial),data=sp_dups_df2, REML=F)

  # models_list<-list(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2f,mnull)
  # modelsR<-lapply(models_list,rsquared.glmm)
  # modelsRsq <- matrix(unlist(modelsR), ncol=6, byrow=T)
  # 
  # msAICc <- model.sel(m1,m1a,m1b,m1c,m1d,m1e,m1f,m2,m2a,m2b,m2c,m2d,m2e,m2)
  ...
  # m1d_w[i]<-subset(msAICc, model=="m1d")$weight
  # m1e_w[i]<-subset(msAICc, model=="m1e")$weight
  # m1f_w[i]<-subset(msAICc, model=="m1f")$weight
  # 
  # m2_w[i]<-subset(msAICc, model=="m2")$weight
  # m2a_w[i]<-subset(msAICc, model=="m2a")$weight
  # m2b_w[i]<-subset(msAICc, model=="m2b")$weight
  # m2c_w[i]<-subset(msAICc, model=="m2c")$weight
  # m2d_w[i]<-subset(msAICc, model=="m2d")$weight
  # m2e_w[i]<-subset(msAICc, model=="m2e")$weight
  # m2f_w[i]<-subset(msAICc, model=="m2f")$weight
  # #mnull_w[i]<-subset(msAICc, model=="mnull")$weight
  # m3_w[i]<-subset(msAICc, model=="m3")$weight
  # # m4_w[i]<-subset(msAICc, model=="m4")$weight
  # # m5_w[i]<-subset(msAICc, model=="m5")$weight
  # 
  # 
  ...
  # marg_Rsq_m1d[i]<-modelsRsq[5,4]
  # marg_Rsq_m1e[i]<-modelsRsq[6,4]
  # marg_Rsq_m1f[i]<-modelsRsq[7,4]
  # 
  # marg_Rsq_m2[i]<-modelsRsq[8,4]
  # marg_Rsq_m2a[i]<-modelsRsq[9,4]
  # marg_Rsq_m2b[i]<-modelsRsq[10,4]
  # marg_Rsq_m2c[i]<-modelsRsq[11,4]
  # marg_Rsq_m2d[i]<-modelsRsq[12,4]
  # marg_Rsq_m2e[i]<-modelsRsq[13,4]
  # marg_Rsq_m2f[i]<-modelsRsq[14,4]
  # 
  # marg_Rsq_mnull[i]<-modelsRsq[16,4]
  # 
  #   # marg_Rsq_m4[i]<-modelsRsq[16,4]
  # # marg_Rsq_m5[i]<-modelsRsq[17,4]
  # 
  # cond_Rsq_m1[i]<-modelsRsq[1,5]
  # cond_Rsq_m1a[i]<-modelsRsq[2,5]
  # cond_Rsq_m1b[i]<-modelsRsq[3,5]
  # cond_Rsq_m1c[i]<-modelsRsq[4,5]   #should be 4 without all other models '#'d out
  # cond_Rsq_m1d[i]<-modelsRsq[5,5]
  # cond_Rsq_m1e[i]<-modelsRsq[6,5]
  # cond_Rsq_m1f[i]<-modelsRsq[7,5]
  # 
  # cond_Rsq_m2[i]<-modelsRsq[8,5]
  # cond_Rsq_m2a[i]<-modelsRsq[9,5]
  # cond_Rsq_m2b[i]<-modelsRsq[10,5]
  # cond_Rsq_m2c[i]<-modelsRsq[11,5]
  # cond_Rsq_m2d[i]<-modelsRsq[12,5]
  # cond_Rsq_m2e[i]<-modelsRsq[13,5]
  # cond_Rsq_m2f[i]<-modelsRsq[14,5]
  # cond_Rsq_m3[i]<-modelsRsq[15,5]
  # cond_Rsq_mnull[i]<-modelsRsq[16,5]

   
  # cond_Rsq_m4[i]<-modelsRsq[16,5]
  # cond_Rsq_m5[i]<-modelsRsq[17,5]
...
#   AIC_m5[i]<-AIC(m5)
  ...
  # var_imp<-summary(model.avg(models_list, subset = delta < 2))
  # #x<-model.avg(models_list, subset = delta < 7)
  ...
  # MTV_i[i]<-var_imp$importance["mean_var_scale"]
  # MTC_MTV_i[i]<-var_imp$importance["mean_slope_scale:mean_var_scale"]
  # LUC_MTV_i[i]<-var_imp$importance["change_rate_scale:mean_var_scale"]
  ...
  # MTV_av[i]<-var_imp$coefmat.subset["mean_var_scale","Estimate"]
  # MTC_MTV_av[i]<-var_imp$coefmat.subset["change_rate_scale:mean_var_scale","Estimate"]
  # LUC_MTV_av[i]<-var_imp$coefmat.subset["mean_slope_scale:mean_var_scale","Estimate"]
 ...
 #For plotting predicted values against real values

 #pred<-predict(avg.top.models.AIC)
 
 #dt3<-dt[dt$ID %in% dt2$ID]
 #real<-dt3$TL
```
...
AIC_df<-data.frame(cbind(AIC_m1,AIC_m1a, AIC_m1b, AIC_m1c, AIC_m1d, AIC_m1e, AIC_m1f, AIC_m2,AIC_m2a, AIC_m2b, AIC_m2c, AIC_m2d, AIC_m2e, AIC_m2f,AIC_m3,AIC_m4,AIC_mnull))

AIC_del<-AIC_df[,c(1:16)] - AIC_df$AIC_mnull
...
AIC_df<-data.frame(cbind(AIC_m1,AIC_m1a, AIC_m1b, AIC_m1c, AIC_m1d, AIC_m1e, AIC_m1f,AIC_m1g, AIC_m2,AIC_m2a, AIC_m2b, AIC_m2c, AIC_m2d, AIC_m2e, AIC_m2f,AIC_m3,AIC_m4, AIC_mnull))


AIC_del<-AIC_df[,c(1:17)] - AIC_df$AIC_mnull
...
mean_i<- c(mean(LUC_i),mean(MTC_i),mean(MTV_i),mean(LUC_MTV_i),mean(LUC_MTC_i),mean(MTC_MTV_i))
lowCI_i<-c(sort(LUC_i)[250], sort(MTC_i)[250], sort(MTV_i)[250], sort(LUC_MTV_i)[250], sort(LUC_MTC_i)[250], sort(MTC_MTV_i)[250])
highCI_i<-c(sort(LUC_i)[9750], sort(MTC_i)[9750], sort(MTV_i)[9750], sort(LUC_MTV_i)[9750], sort(LUC_MTC_i)[9750], sort(MTC_MTV_i)[9750])

Variable<-c("LUC", "MTV", "MTC", "LUC*MTV", "LUC*MTC", "MTV*MTC")

conf_i<-data.frame(rbind( lowCI_i, mean_i, highCI_i))
colnames(conf_i)<-Variable
conf_i

library(plotrix)

plotCI(1:6, mean_i, (highCI_i-mean_i), (mean_i-lowCI_i), ylab="Variable Importance", xlab="" ,xaxt = "n", 
main="Variable Importance", lwd=1)
axis(1, at=1:6, labels=colnames(conf_i), las=2)
abline(h=0, col="red", lty =2)


##For averaged model - conditional estimates
 
  
mean_av<-c(mean(int_av), mean(LUC_av), mean(MTV_av),mean(MTC_av), mean(LUC_MTV_av), mean(LUC_MTC_av), mean(MTC_MTV_av))
 
lowCI_av<-c(sort(int_av)[250],sort(LUC_av)[250],sort(MTV_av)[250],sort(MTC_av)[250],sort(LUC_MTV_av)[250],sort(LUC_MTC_av)[250],sort(MTC_MTV_av)[250])

highCI_av<-c(sort(int_av)[9750],sort(LUC_av)[9750],sort(MTV_av)[9750],sort(MTC_av)[9750],sort(LUC_MTV_av)[9750],sort(LUC_MTC_av)[9750],sort(MTC_MTV_av)[9750])

Variable<-c( "Intercept", "LUC", "MTV", "MTC", "LUC*MTV", "LUC*MTC", "MTV*MTC")
...
# par(mar=c(14,5,1,1))
# par(mgp=c(2, 1,0))
plotCI(1:7, mean_av, (highCI_av-mean_av), (mean_av-lowCI_av), ylab="Coefficient Values", xlab="" ,xaxt = "n", main="Model Averaged Coefficients", lwd=1)
axis(1, at=1:7, labels=colnames(conf_av), las=2)
abline(h=0, col="red", lty =2)
...
LowCI_RSq<-c(sort(marg_Rsq_m1)[250],sort(cond_Rsq_m1)[250])   #should be 250 for R=9999
HighCI_RSq<-c(sort(marg_Rsq_m1)[9750],sort(cond_Rsq_m1)[9750]) #should be 9750 for R=9999

RSq_int<-rbind(LowCI_RSq,Mean_RSq,HighCI_RSq)
colnames(RSq_int)<-Var_RSq
RSq_int
...
####average of two top models

mean_conf<-c(mean(interceptw), mean(change_ratew), mean(mean_sew),mean(mean_slopew), mean(change_mean_sew), mean(change_mean_slopew), mean(mean_se_mean_slopew))

lowCI<-c(sort(interceptw)[250],sort(change_ratew)[250],sort(mean_sew)[250],sort(mean_slopew)[250],sort(change_mean_sew)[250],sort(change_mean_slopew)[250],sort(mean_se_mean_slopew)[250])

highCI<-c(sort(interceptw)[9750],sort(change_ratew)[9750],sort(mean_sew)[9750],sort(mean_slopew)[9750],sort(change_mean_sew)[9750],sort(change_mean_slopew)[9750], sort(mean_se_mean_slopew)[9750])
...
mean(AIC_m1)
##
coef_df2<-cbind(intercept2,change_rate2,mean_se2,mean_slope2,change_mean_se2,mean_se_mean_slope2)

####model 1c

Variable2<-c( "Intercept", "Land Use Change", "Mean Temp Var", "Mean Temp Change","Land Use Change*Mean Temp Var", "Mean Temp Var*Mean Temp Change")

mean_conf2<-c(mean(intercept2), mean(change_rate2), mean(mean_se2),mean(mean_slope2), mean(change_mean_se2), mean(mean_se_mean_slope2))

lowCI2<-c(sort(intercept2)[250],sort(change_rate2)[250],sort(mean_se2)[250],sort(mean_slope2)[250], sort(change_mean_se2)[250], sort(mean_se_mean_slope2)[250])

highCI2<-c(sort(intercept2)[9750],sort(change_rate2)[9750],sort(mean_se2)[9750],sort(mean_slope2)[9750], sort(change_mean_se2)[9750], sort(mean_se_mean_slope2)[9750])

conf2<-data.frame(rbind(lowCI2, mean_conf2, highCI2))
colnames(conf2)<-Variable2
conf2

RSq_df1c<-cbind(marg_Rsq_m1c,cond_Rsq_m1c)
Var_RSq<-c("Marginal R Squared","Conditional R Squared")
Mean_RSq1c<-c(mean(marg_Rsq_m1c),mean(cond_Rsq_m1c))
LowCI_RSq1c<-c(sort(marg_Rsq_m1c)[250],sort(cond_Rsq_m1c)[250])
HighCI_RSq1c<-c(sort(marg_Rsq_m1c)[9750],sort(cond_Rsq_m1c)[9750])


RSq_1c<-rbind(LowCI_RSq1c, Mean_RSq1c, HighCI_RSq1c)
colnames(RSq_1c)<-Var_RSq
RSq_1c


plotCI(1:6, mean_conf2, (highCI2-mean_conf2), (mean_conf2-lowCI2), ylab="Coefficient Values", xlab="" ,xaxt = "n")
axis(1, at=1:6, labels=colnames(conf2), las=2)
abline(h=0, col="red", lty =2)

hist(AIC_m1c, breaks=100)
mean(AIC_m1c)
