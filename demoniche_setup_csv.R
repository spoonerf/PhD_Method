                              ...
                              sumweight = FALSE, spin_years = spin_years, dispersal_probabilities)
{
  ...
  # if (file.exists(paste(wd,"/Big_Files/dist_pops_all.nc", sep=""))){
  # load("dist_pops_all.nc")
  # 
  # if (file.exists("dist_pops_all.nc")){
  #   dist_populations<- nc_open("dist_pops_all.nc")
  #   #dist_populations<-Matrix(dist_populations, sparse = TRUE)
  # } else {
  #   dist_populations <- spDists(as.matrix(Niche_ID[, 2:3]), longlat = TRUE)
  #   dimnames(dist_populations) <- list(Niche_ID[, 1], Niche_ID[,1])
  #   save(dist_populations, file = "dist_pops.RData")
  #     }
  #   
  ...
  # if (file.exists("dist_latlong.RData")){
  #   load("dist_latlong.RData")
  #   #dist_latlong<-Matrix(dist_latlong, sparse = TRUE)
  # } else {
  #   dist_latlong <- round(as.matrix(dist(Niche_ID[, 2:3])), 1)
  #   save(dist_latlong, file = "dist_latlong.RData")   #save as netcdf
  # }
  
  dist_latlong <- round(as.matrix(dist(Niche_ID[1:(length(unique(Niche_ID[,2]))+2), 2:3])), 1)
  #dist_latlong <- round(as.matrix(dist(Niche_ID[, 2:3])), 1)  #just need enough to do more than one row of longitude
