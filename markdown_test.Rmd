...
>>>>>>> 11e9115341220c907d58de3492009b9d2c60ede1
...
c_ibex_k_16000 <- demoniche_model_me(modelname = "Capra_k_16000", Niche = TRUE, 
                                     Dispersal = TRUE, repetitions = 1,
                                     foldername = paste("/Demoniche_Repetitions/rep_",i, sep=""))
}


lapply(reps, rep_demoniche)


library(doParallel)
registerDoParallel(cores=2)
foreach(i=1:10) %dopar% rep_demoniche(i)
stopCluster(cl)
...
plot_rep<-function(proj_path){
  ...
  load(proj_path)
  id<-as.numeric(gsub("\\D", "", proj_path)) 
  p<-Projection
  trend<-colSums(p[1,1,1:530,])
  tid<-cbind(id, trend)
  return(tid)
}
...
rep_df$year<-as.numeric(gsub("Year_", "", year))
rep_df$id<-as.factor(rep_df$id)

library(ggplot2)
library(dplyr)
ggplot(rep_df, aes(x = year, y= log10(trend), group=id))+
  geom_line( alpha=0.5)
