...
#write.csv(bg_pse, "background_random_points.csv")
bg_pse<-read.csv("background_random_points.csv")
bg_pse<-bg_pse[,-1]
...
evaluate(spxy, bg, wm)
threshold(evl_bc[[1]])['spec_sens']
threshold(evl_rf[[2]])['spec_sens']
threshold(evl_gam[[2]])['spec_sens']
...
dir.create(paste(wd,"/Demoniche_Repetitions/Kern_kap/max_disp_",max_disp,sep=""),showWarnings = TRUE)
...
c_ibex_k_16000 <- demoniche_model_me(modelname = "Capra_k_16000", Niche = TRUE, 
                                     Dispersal = TRUE, repetitions = 1,
                                     foldername = paste("Demoniche_Repetitions/Kern_kap/max_disp_",max_disp,"/",i, sep=""))
}
...
library(dplyr)
 ...
 plot_rep<-function(proj_path){
  ...
  id<-gsub("D:/Fiona/Git_Method/Git_Method/Demoniche_Repetitions/Kern_kap/max_disp_", "", proj_path)
...
wd<-getwd()
wd<-paste(wd, "/Demoniche_Repetitions/Kern_kap/", sep="")
...
#write.csv(rep_df_all, "kern_kap_params.csv")
```


```{r, message=FALSE, warning=FALSE}

rep_df_all<-read.csv("kern_kap_params.csv")
