...
Calculating threshold values for each of the kappa, no omission and true skill statistic evaluation methods. These will be used to create prediction maps of presence/absence of Alpine ibex for each year 1950-2016. 
...
Copying the 1950 900 files for the 900 year spin up
...
from_tss<-paste(wd, "/Alp_SDMs/Ensembles_2006_2016/pres_abs_true_skill_stat_weighted_ensemble_sdm_1950.tif", sep="")
to_tss<-paste(wd, "/Alp_SDMs/Ensembles_2006_2016/pres_abs_true_skill_stat_weighted_ensemble_sdm_", spin_years, ".tif", sep="")

file.copy(from_sdm,to_sdm, overwrite = T)
file.copy(from_pa,to_pa, overwrite = T)
file.copy(from_kap,to_kap, overwrite = T)
file.copy(from_tss, to_tss, overwrite = T)
...
fake_spin_up<-matrix(rep(fake_map_mine[,4], 899), nrow=nrow(fake_map_mine))
...
plot(n_2000[900:967], type="l", ylab="Number of Alpine Ibex", xlab="")

10^mean(diff(log10(n_2000[900:966])))
...
```{r dispersal kernel shape - run models, message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE, eval=FALSE}
#setwd("C:Users/Fiona/Documents/Demoniche")
...
kern_mat<-matrix(c(rep(c(1,1,0.1,1), (nrow(var_grid)/4)),rep(c(1,0.1,1,1), (nrow(var_grid)/4)),rep(c(1,1,1,1),(nrow(var_grid)/4)),rep(c(1,1,1,5), (nrow(var_grid)/4))), ncol=4, byrow=T)
...
dir.create(paste("D:/Fiona/Git_Method/Git_Method/Demoniche_Repetitions/Three_Vars_kern/Kern_", s,"_LDD_",LDD,"_SD_", SD,sep=""))
...
c_ibex_k_16000 <- demoniche_model_me(modelname = "Capra_k_16000", Niche = TRUE, 
                                     Dispersal = TRUE, repetitions = 1,
                                     foldername = paste("Demoniche_Repetitions/Three_Vars_kern/Kern_", s,"_LDD_",LDD,"_SD_", SD,"/rep",i,sep=""))
}
...
```{r dispersal kernel shape - extract trends,message=FALSE, warning=FALSE, eval=FALSE}
library(ggplot2)
library(dplyr)
...
#path<-paste(f, lf, sep="")
  ...
  load(proj_path)
  kern_id<-strsplit(proj_path, "[_/]")[[1]][13]
  ldd_id<-strsplit(proj_path, "[_/]")[[1]][15]
  sd_id<-strsplit(proj_path, "[_/]")[[1]][17]
  rep_id<-strsplit(proj_path, "[_/]")[[1]][18]
  p<-Projection
  trend<-colSums(p[1,1,1:length(p[1,1,,1]),])
  tid<-cbind(kern_id,ldd_id, sd_id, rep_id, trend)
  return(tid)
  }
...
write.csv(rep_df_all, "threevars_kern_sd_ldd.csv")
```

```{r}

rep_df_all<-read.csv("threevars_kern_sd_ldd.csv")


rep_df_all$kern_id<-as.numeric(rep_df_all$kern_id)

rep_df_all$kern_mat[rep_df_all$kern_id<=18]<-"1,1,0.1,1"
rep_df_all$kern_mat[rep_df_all$kern_id>18 & rep_df_all$kern_id<=36]<-"1,0.1,1,1"
rep_df_all$kern_mat[rep_df_all$kern_id>36 & rep_df_all$kern_id<=54]<-"1,1,1,1"
rep_df_all$kern_mat[rep_df_all$kern_id>54 & rep_df_all$kern_id<=72]<-"1,1,1,5"

rep_df_all$kern_mat<-as.factor(rep_df_all$kern_mat)

rep_df_all$trend<-as.numeric(as.character(rep_df_all$trend))

rep_df_all$sd_id_f<-factor(rep_df_all$sd_id, levels=c("0.25","0.375","0.5","0.625","0.75","1"))
...
p01<-p01 + facet_grid(kern_mat ~ sd_id_f, labeller = label_both)
p01
```

```{r}
#rep_df_short<-rep_df_all[rep_df_all$year >1949 & rep_df_all$ldd_id==0.9 & rep_df_all$rep_id=="rep1" & rep_df_all$sd_id_f == 0.25,]
rep_df_short<-rep_df_all[rep_df_all$year >1949 ,]

library(ggplot2)
#library(patchwork)

p01<-ggplot(rep_df_short, aes(x=year, y=trend, colour=ldd_id_f))+
  geom_line()
#p01
