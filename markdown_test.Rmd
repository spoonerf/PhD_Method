...
library(dplyr)
 ...
 plot_rep<-function(proj_path){
  ...
  load(proj_path)
  id<-gsub("C:/Users/Fiona/Documents/PhD/PhD_Method/Demoniche_Repetitions/Kern/max_disp_", "", proj_path)
  id<-gsub("Projection_rep1_Reference_matrix.rda", "",id)
  md_id<-as.numeric(substr(id, 1, nchar(id)-3))
  rep_id<-as.numeric(substr(id, nchar(id)-1,nchar(id)-1 ))
  p<-Projection
  trend<-colSums(p[1,1,1:length(p[1,1,,1]),])
