...
start.time <- Sys.time()
 ...
 max_disp<-as.character(kern_mat[s,2])
  dir.create(paste(demoniche_folder,"/new_patch_disp_dens",max_disp,sep=""),showWarnings = TRUE)
...
rep_demoniche<-function(i){
 ...
 demoniche_setup_me(modelname = binomial ,Populations = Populations, Nichemap = patch_spin_up,
                matrices = matrices,matrices_var = matrices_var, prob_scenario = prob_scenario,
                  ...
                  density_individuals = 2671,   #density_mine
...
c_ibex_k_16000 <- demoniche_model_me(modelname = binomial, Niche = TRUE, 
                                     ...
                                     foldername = paste(binomial,"/Demoniche_Output/new_patch_disp_dens",max_disp,"/",i, sep=""))
}
...
```{r,message=FALSE, warning=FALSE, echo=FALSE}
 ...
 plot_rep<-function(proj_path){
  ...
  load(proj_path)
  id<-gsub(paste(demoniche_folder, "/new_patch_disp_dens", sep=""), "", proj_path)
...
write.csv(rep_df_all, paste(species_directory, "my_disp_dens_function.csv", sep="/"))
```
...
highfoldernames<-highfoldernames[5:8]
...
convert_pop_out<-function(foldername){
  ...
  max_disp<-strsplit(foldername, "[/_]")[[1]][4]
  rep_id<-strsplit(foldername, "[/_]")[[1]][5]
...
b<-read.csv(paste(demoniche_folder ,"new_patch_disp_113000/",x,"/Reference_matrix_pop_output.csv", sep="/"))
b<-b[,-1]
...
#writeRaster(all_years_stack, "Alpine_Ibex_Projection.tif", overwrite=T)
```
