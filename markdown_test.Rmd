...
colnames(fake_spin_up)[4:length(colnames(fake_spin_up))]<-col_years<-paste("Year_", 1850:2016, sep="")

plot(years,colMeans(na.omit(niche_map_mine)[4:length(colnames(niche_map_mine))]), type="l", ylab="Mean suitability index")
plot(1850:2015,colMeans(na.omit(fake_spin_up)[4:length(colnames(fake_spin_up))]), type="l", ylab="Mean suitability index")
...
```{r}
library(ggplot2)
rep_df_all<-rep_df_all[rep_df_all$md_id != "1000000" ,]
...
```{r,message=FALSE, warning=FALSE, echo=FALSE}
 ...
 plot_rep<-function(proj_path){
  ...
  load(proj_path)
  id<-gsub(paste("D:/Fiona/Git_Method/Git_Method/Demoniche_Repetitions/Fake_Three_Vars_kern", sep=""), "", proj_path)
  id<-gsub("Projection_rep1_Reference_matrix.rda", "",id)
  md_id<-strsplit(id, "/")[[1]][1]
  rep_id<- gsub(".*[/]([^/]+)[/]*", "\\1", id)
  p<-Projection
...
folders<-list.files("D:/Fiona/Git_Method/Git_Method/Demoniche_Repetitions/Fake_Three_Vars_kern")

for (folder in folders){

  print(folder)

  f<-paste("D:/Fiona/Git_Method/Git_Method/Demoniche_Repetitions/Fake_Three_Vars_kern",folder, sep="/")
...
write.csv(rep_df_all,  "my_fake_function_test.csv")
```
...
rep_df_all<-read.csv( "my_fake_function_test.csv")
rep_df_all$rep_id<-factor(rep_df_all$rep_id)
rep_df_all$md_id<-factor(rep_df_all$md_id)
