...
years<-as.character(1954:2016) #should be 1950
...
gam_demon<-function(x){
  
  #df<-melt_short[melt_short$sp_lpi.ID ==x,]
  df<-melt_lambda_short[melt_lambda_short$sp_lpi.ID ==x,]
  
#not sure what this does - adding a constant of 1 so that logging doesn't go weird?
  if (sum(na.omit(df$value<1))>0) {
    df$value<-df$value+1
  } 
  ...
  if (points >=6) {           
    ...
    ial<-data.frame(x, unique(Year),pv2)
