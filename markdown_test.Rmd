...
density_individuals <- 4000  #4292.32 to 16096.2 based on density being between 8 and 30 per 100 ha and the area of each cell being 53654 ha - K_weight<-c(rep(1, length(stages)))  #the weight with which carrying capacity affects each stage was FALSE
...
write.csv(df, paste(species_directory, "Capra_ibex_gbif.csv", sep="/"), row.names = FALSE)
```

```{r Plotting occurrence points, message=FALSE, warning=FALSE}
df<-read.csv(paste(species_directory, "Capra_ibex_gbif.csv", sep="/") )
...
```{r Presence Data, cache=TRUE, message=FALSE, warning=FALSE}
...
```{r Historical predictions, cache=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r Scaling Carrying Capacity, cache=TRUE, message=FALSE, warning=FALSE, eval=FALSE, echo=FALSE}
...
#write.csv(rep_df_all, paste(species_directory, "kern_kap_new_params.csv", sep="/")
```
