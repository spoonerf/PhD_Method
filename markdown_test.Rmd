...
write.csv(sam, paste(species_directory, "background_random_points2.csv", sep="/"))
...
start.time <- Sys.time()
 ...
 max_disp<-as.character(kern_mat[s,2])
  dir.create(paste(demoniche_folder,"/new_patch_disp_",max_disp,sep=""),showWarnings = TRUE)
...
c_ibex_k_16000 <- demoniche_model_me(modelname = binomial, Niche = TRUE, 
                                     ...
                                     foldername = paste(binomial,"/Demoniche_Output/new_patch_disp_",max_disp,"/",i, sep=""))
}
...
```{r,message=FALSE, warning=FALSE, echo=FALSE}
 ...
 plot_rep<-function(proj_path){
  ...
  load(proj_path)
  id<-gsub(paste(demoniche_folder, "/new_patch_disp_", sep=""), "", proj_path)
...
write.csv(rep_df_all, paste(species_directory, "my_disp_function.csv", sep="/"))
```
...
rep_df_small<-rep_df_all[rep_df_all$md_id != "1000000",]
library(ggplot2)
...
#highfoldernames<-highfoldernames[1:3]
