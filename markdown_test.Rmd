...
tr_gam_noem<-threshold(gam_ev, 'no_omission')
...
tr_rf_noem<-threshold(rf_ev, 'no_omission')
tr_rf_k<-threshold(rf_ev, 'kappa')
tr_rf_tss<-rf_ev@t[which.max(rf_ev@TPR + rf_ev@TNR -1)]

thresh_noem<-mean(c(tr_bc_noem, tr_gam_noem, tr_rf_noem))
...
tholds<-c(thresh_noem, thresh_weighted_k, thresh_weighted_tss)
...
for (i in 1:length(years)){
 ...
 writeRaster(wm , paste(wd, "/Alp_SDMs/Ensembles/weighted_ensemble_sdm_", years[i], ".tif", sep=""), overwrite=TRUE)
  writeRaster(pa_noem , paste(wd, "/Alp_SDMs/Ensembles/pres_abs_no_emission_weighted_ensemble_sdm_", years[i], ".tif", sep=""), overwrite=TRUE)
