...
tg<-brick(paste(wd, "/tg_0.25deg_reg_v15.0.nc/tg_0.25deg_reg_v15.0.nc", sep="")) #mean temp
tn<-brick(paste(wd, "/tn_0.25deg_reg_v15.0.nc/tn_0.25deg_reg_v15.0.nc", sep="")) #min temp
tx<-brick(paste(wd, "/tx_0.25deg_reg_v15.0.nc/tx_0.25deg_reg_v15.0.nc", sep="")) #max temp
...
k<-5000   #need to find a better way of estimating this
lin<-function(x){
  x*k
  }
...
xy<-Populations[,c(2,3)]

hsi<-extract(sdms, xy)


link<-lin(hsi)
link_spin_up<-matrix(rep(link[,1], 900), ncol=900)
link_spin_up_comb<-cbind(link_spin_up,link)

sigk<-sig(hsi)
sigk_spin_up<-matrix(rep(sigk[,1], 900), ncol=900)
sigk_spin_up_comb<-cbind(sigk_spin_up,link)

ltk<-lt(hsi)
ltkk_spin_up<-matrix(rep(ltk[,1], 900), ncol=900)
ltkk_spin_up_comb<-cbind(ltkk_spin_up,link)
...
HSM as patch - Carrying capacity null -  transition matrix NULL?
```{r}
demoniche_setup(modelname = "patch_ltk_low_disp",Populations = Populations, Nichemap = patch_map_mine,
                ...
                no_yrs = no_yrs_mine, K=ltk, Kweight = K_weight, 
                sumweight =sumweight)

source("demoniche_model_me.R")
patch_run <- demoniche_model_me(modelname = "patch_ltk_low_disp", Niche = TRUE, 
                                     Dispersal = TRUE, repetitions = 1,
                                     foldername = "patch_ltk_low_disp")

plot(years,patch_run[,"Meanpop","Reference_matrix"], type="l", xlim=c(1955,2016))

```

```{r}
demoniche_setup(modelname = "patch_ltk_high_disp",Populations = Populations, Nichemap = patch_map_mine,
                matrices = matrices,matrices_var = matrices_var, prob_scenario = prob_scenario,
                stages = stages, proportion_initial = proportion_initial,
                density_individuals = density_individuals,
...
source("demoniche_model_me.R")
patch_run <- demoniche_model_me(modelname = "patch_ltk_high_disp", Niche = TRUE, 
                                     Dispersal = TRUE, repetitions = 1,
                                     foldername = "patch_ltk_high_disp")
...
pop_out<-read.csv(paste(wd, "/CIbex_disp_niche/pop_output.csv", sep=""), header=T)
pop_out<-pop_out[,-1]
