...
niche_spin_up<-matrix(rep(niche_map_mine[,4], 499), nrow=nrow(niche_map_mine))
niche_spin_up<-cbind(niche_map_mine[,1:3],niche_spin_up, niche_map_mine[,4:ncol(niche_map_mine)])

patch_spin_up<-matrix(rep(patch_map_mine[,4], 499), nrow=nrow(patch_map_mine))
patch_spin_up<-cbind(patch_map_mine[,1:3],patch_spin_up, patch_map_mine[,4:ncol(patch_map_mine)]) #last patch used to be niche

fake_spin_up<-matrix(rep(fake_map_mine[,4], 499), nrow=nrow(fake_map_mine))
...
col_years<-paste("Year_", 1450:2016, sep="")
...
dir.create(paste(wd,"/Demoniche_Repetitions/Kern/max_disp_",max_disp,sep=""),showWarnings = TRUE)
...
library(dplyr)
 ...
 plot_rep<-function(proj_path){
  ...
  load(proj_path)
  id<-strsplit(proj_path, "/")[[1]]
  md_id<-gsub("max_disp_","",id[length(id)-2])
  rep_id<-id[length(id)-1]
  p<-Projection
  trend<-colSums(p[1,1,1:length(p[1,1,,1]),])
...
wd<-getwd()
wd<-paste(wd, "/Demoniche_Repetitions/Kern/", sep="")
...
write.csv(rep_df_all, "kern_params.csv")
```


```{r}

rep_df_all<-read.csv("kern_params.csv")
...
rep_df_short<-rep_df_all[rep_df_all$year>1949 & rep_df_all$year<2016,]
...
p01<-ggplot(rep_df_short, aes(x=year, y=trend, group=interaction(rep_id, md_id),colour=md_id))+
  geom_line()+ 
   theme(text = element_text(size=9))+ 
...
p01<-ggplot(rep_df_short, aes(x=year, y=trend, group=interaction(rep_id, md_id),colour=md_id))+
  ...
  #facet_grid(kern_mat ~ sd_id_f, labeller = label_context)+
   theme(text = element_text(size=9)) + 
  labs(color='Maximum dispersal distance (km)') 
p01
...
convert_pop_out<-function(foldername){
  ...
  pop_out<-read.csv(paste(wd,"/Demoniche_repetitions/Kern/" ,foldername, "/pop_output.csv", sep=""), header = TRUE)
  pop_out<-pop_out[,-1]
...
>>>>>>> 0457f95e7bfeb6301f73125bb26ec9d4023f8355
...
ggplot(both_df, aes(x= year, y=value, group=interaction(rep_id, max_disp), colour= alpine.ID))+
