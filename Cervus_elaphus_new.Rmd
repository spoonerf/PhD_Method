...
hyde_files<-list.files(paste(wd,"/hyde/", sep=""))

hyde_resample<-stack(paste(wd,"/hyde/", hyde_files,sep=""))
...
link_spin[is.na(link_spin)]<-0.6188643*carry_k
...
for (s in 1:nrow(var_grid)){
  ...
  matrices_var<-matrix(0.1, ncol = 1, nrow = nrow(matrices), dimnames = list(NULL, "sd"))

start.time <- Sys.time()

 max_disp<-as.character(kern_mat[s,1])
  dir.create(paste(demoniche_folder,"/hyde_new_patch_disp_test_",max_disp,sep=""),showWarnings = TRUE)
...
c_ibex_k_16000 <- demoniche_model_me(modelname = binomial, Niche = TRUE,
                                     ...
                                     foldername = paste(binomial,"/Demoniche_Output/hyde_new_patch_disp_test_",max_disp,"/",i,sep=""))
