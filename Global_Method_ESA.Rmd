...
LPI_pop<-subset(LPI, Specific_location==1 & System !="Marine" & (Class =="Aves"| Class=="Mammalia") & ID !=4438)
...
points_per_pop1950_2015 = rowSums(pop_datab)
length_id <- data.frame(ID,points_per_pop1950_2015)

LPI_EU<-merge(length_id, LPI_pop, by = "ID")
LPI_EU<-subset(LPI_EU, points_per_pop1950_2015 >=2)

#LPI_EU2<-LPI_EU[,c(1:3,64:126)]
LPI_EU2<-LPI_EU[,c(1:3,66:131)]
LPI_EU2[LPI_EU2 == 'NULL'] = NA
pop_data<- LPI_EU2[,c(1,4:69)]

nrow(LPI_EU2)

#need to remove trends which end before 1992
#as.numeric(as.character(pop_data[1,c(2:57)]))

years<-1950:2015
m<-as.matrix(pop_data[,c(2:67)]) 

#apply(m,1,function(x) {years[max(which(x !="NULL"))]})

last_y<-numeric()
for (i in 1:nrow(m)){
  r<-pop_data[i,c(2:67)]
  y<-years[max(which(r !="NULL"))]
  id<-pop_data[i,1]
  yid<-data.frame(id,y)
  last_y<-rbind(last_y, yid)
  print(i)
  }


LPI_df<-merge(LPI_EU2, last_y, by.x = "ID", by.y="id")

LPI_EU2<-LPI_df[LPI_df$y>1992,]
...
doFit = function(sp_name) {
  ...
  name<-spid2$Binomial
  id<-spid2$ID
  points<-spid2$points_per_pop1950_2015
  ...
  if (points >=6) {           
    ...
    pv2$fit<-pv2$fit[which(df$Year == 1992):which(df$Year == last_year)] 
    }
    ...
    if (min(df$Year<=1992)){
    last_year = max(df$Year)
    Pop_interp2<-Pop_interp2[which(df$Year == 1992):which(df$Year == last_year)] 
    }
...
write.csv(all_df, "Global_Population_Trends_Rsq_Lambda_17_06_27_ESA.csv") ##updated LPI

#write.csv(all_df, "global_pops_2005.csv")
...
LPIsp<-subset(LPI, Specific_location==1 & System !="Marine" & (Class =="Aves"| Class=="Mammalia") & ID !=4438)
...
write.csv(mean_df, "All_LPI_nobuff_1931_mean_temp_sum_change.csv")
```
...
sum(AIC_del$MTC < 0)/length(AIC_del$MTC)
