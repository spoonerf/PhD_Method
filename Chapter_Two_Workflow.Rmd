...
colnames(glob_clim)[2]<-"CRUTEM_Estimate_ESA"
...
#Europe
```{r}
df_eu<-subset(df, !is.na(lambda_mean)& !is.na(EOBS_Estimate) & !is.na(CRUTEM_Estimate_ESA) & !is.na(HILDA_change) & !is.na(HYDE_change)& !is.na(LUH_change)& !is.na(Binomial) & length_time >= 5 & (Class=="Aves" | Class == "Mammalia" ) & Specific_location ==1)#& r_sq >0.5)

# df_eu$Protected_status[df_eu$Protected_status == "No (area surrounding PA)"] <- "No"
# df_eu$Protected_status[df_eu$Protected_status == "No (large survey area)"] <- "No"
...
parm_df<-df_glb[,c("ID", "CRUTEM_Estimate","HYDE_change", "LUH_change","Log_Body_Mass_g")]  ##ID, land use, and climate
...
dt_glb$Binomial<-as.character(dt_glb$Binomial)
dt_eu$Binomial<-as.character(dt_eu$Binomial)

dt_glb$Class<-as.character(dt_glb$Class)
dt_eu$Class<-as.character(dt_eu$Class)

dt_glb$Protected_status<-as.character(dt_glb$Protected_status)
...
loop_grid_eu<-expand.grid(climate, lu, class)
colnames(loop_grid_eu)<-c("climate", "lu", "class")
...
class<-c("Aves", "Mammalia")
loop_grid<-expand.grid(climate, lu, class)
colnames(loop_grid)<-c("climate", "lu", "class")
...
for (i in 1:nrow(loop_grid)){
  ...
  df_sel<-cbind(dt_glb$lambda_mean,dt_glb$loc_id,dt_glb$Binomial, dt_glb$Log_Body_Mass_g_scale,dt_glb$Class, dt_glb$Protected_status)
  df_sel2<-subset(dt_glb, select=c(sel_clim,sel_lu))
