...
doMean = function(sp_name) {
  spid2 = subset(LPI_EU, ID == sp_name)   #subsetting the population data by each population
...
all_df_list <- lapply(unique(LPI_EU$ID), doMean)
...
ID<-grid_crop2[i,1]
  ...
  spid = grid_crop2[i,68:130]                     #subsetting only the dates
...
clim<-read.csv(paste(wd,"/Chapter_Two/Europe/" ,"Mean_Temp_E-OBS_50k_buff_na_omitted.csv", sep=""))
