...
```{r,eval=FALSE}
write.csv(all_df, paste(wd,"/Chapter_Two/Europe/", "Global_Population_Lambdas.csv"))
...
##for 1992
#pop_data<-LPI_pop[,c(1,107:120)]

pop_datab <- (pop_data [,2:ncol(pop_data)] !="NULL")
points_per_pop1950_2012 = rowSums(pop_datab)
length_id <- data.frame(ID,points_per_pop1950_2012)
...
for (i in 1:length(grid_crop2$ID)){
  ...
  spid = grid_crop2[i,111:124]                     #subsetting only the dates
  colnames(spid)<-1992:2005
...
write.csv(result, "Hyde_crop_pasture_annual_change_sum_1992.csv")
...
```{r}
lpi<-read.csv("LPI_pops_20160523_edited.csv")
lpi<-lpi[,c(1,2,15,31,32,33,35,136)]
...
luc_hyde<-read.csv("Hyde_crop_pasture_annual_change.csv")
...
ESA data (1992 onwards)

```{r}
pop_esa<-read.csv("Global_Population_Trends_Rsq_Lambda_17_06_27_ESA.csv")
pop_esa<-pop_esa[,c(2,5,8,9)]
colnames(pop_esa)<-c("ID", "r_sq_esa", "lambda_mean_esa", "length_time_esa")

glob_clim_esa<-read.csv("All_LPI_nobuff_1931_mean_temp_sum_change_1992_onwards.csv")
glob_clim_esa<-glob_clim_esa[,c(2,5)]
colnames(glob_clim_esa)[2]<-"CRUTEM_Estimate_ESA"
...
# df_esa<-merge(merge(luc_esa, lpi, by="ID", all=T), merge(pop_esa, glob_clim_esa, by="ID", all=T))
# df<-merge(df, df_esa, by="ID", all=T)
# 
# head(df)



df_global<-join_all(list(pop,glob_clim,luc_hyde,luc_luh,bm[,2:3]),by='ID',type='left')

df<-join(df_global, lpi, by="ID", type="left")


df_esa<-join_all(list(pop_esa, glob_clim_esa,luc_esa,bm[,2:3]),by='ID',type='left')

df_esa<-join(df_esa, lpi, by="ID", type="left")

#eu_clim,luc_hilda,pop_esa,glob_clim_esa,luc_esa

```

#Europe
```{r}
df_esa<-subset(df_esa, !is.na(lambda_mean_esa)& !is.na(CRUTEM_Estimate_ESA) & !is.na(ESA_change) & !is.na(Binomial) & length_time_esa >= 5 & Protected_status != "Unknown"  & Protected_status != "Both" & (Class=="Aves" | Class == "Mammalia" ) & Specific_location ==1& r_sq_esa >=0.499999 & !is.na(Log_Body_Mass_g))

df_esa$Protected_status[df_esa$Protected_status == "No (area surrounding PA)"] <- "No"
df_esa$Protected_status[df_esa$Protected_status == "No (large survey area)"] <- "No"
...
```{r}
df_glb<-subset(df, !is.na(lambda_mean) & !is.na(CRUTEM_Estimate) & !is.na(HYDE_change)& !is.na(LUH_change)& !is.na(Binomial) & length_time >= 5 & Protected_status != "Unknown"  & Protected_status != "Both" & (Class=="Aves" |Class=="Mammalia")&r_sq >=0.4999999 & Specific_location ==1 & !is.na(Log_Body_Mass_g))

df_glb$Protected_status[df_glb$Protected_status == "No (area surrounding PA)"] <- "No"
df_glb$Protected_status[df_glb$Protected_status == "No (large survey area)"] <- "No"
...
sp_dups<-data.frame(ddply(dt,.(Longitude,Latitude),nrow))
sp_dups$loc_id<-1:length(sp_dups$Longitude)
dt<-merge(sp_dups, dt, by=c("Longitude","Latitude"))


##THESE NEED CHANGING
#1950
#parm_df<-dt[,c("ID", "CRUTEM_Estimate","HYDE_change", "LUH_change","Log_Body_Mass_g")]  ##ID, land use, and climate

#1992
parm_df<-dt[,c("ID", "CRUTEM_Estimate_ESA","ESA_change","Log_Body_Mass_g")]  ##ID, land use, and climate

parm_mat<-as.matrix(parm_df)

#1950
#parm_scale_m<-scale(parm_mat[,c( "CRUTEM_Estimate" ,"HYDE_change", "LUH_change","Log_Body_Mass_g")])       #use the scaling factors at the bottom of these to scale the rasters

#1 992
parm_scale_m<-scale(parm_mat[,c( "CRUTEM_Estimate_ESA" ,"ESA_change", "Log_Body_Mass_g")])       #use the scaling 
...
#1950
#colnames(parm_df_scale)<-c("ID","CRUTEM_scale","HYDE_scale","LUH_scale","Log_Body_Mass_g_scale" )
#1992
colnames(parm_df_scale)<-c("ID","CRUTEM_scale","ESA_scale","Log_Body_Mass_g_scale" )

sp_df_scale<-merge(dt, parm_df_scale, by="ID")
...
dt$Binomial<-as.character(dt$Binomial)
#dt_eu$Binomial<-as.character(dt_eu$Binomial)

dt$Class<-as.character(dt$Class)
#dt_eu$Class<-as.character(dt_eu$Class)

dt$Protected_status<-as.character(dt$Protected_status)
#dt_eu$Protected_status<-as.character(dt_eu$Protected_status)
...
EU loop
```{r}
climate<-c("CRUTEM_Estimate_ESA")
lu<-c("ESA_change")
class<-c("Aves", "Mammalia")
loop_grid_esa<-expand.grid(climate, lu, class)
colnames(loop_grid_esa)<-c("climate", "lu", "class")
...
class<-c("Aves", "Mammalia")
loop_grid_glob<-expand.grid(climate, lu, class)
colnames(loop_grid_glob)<-c("climate", "lu", "class")
...
loop_grid<-loop_grid_esa

for (i in 1:nrow(loop_grid)){
  print(i)
  sel_clim<-as.character(loop_grid$climate[i])
  sel_lu<-as.character(loop_grid$lu[i])
  class<-as.character(loop_grid$class[i])
  df_sel<-cbind(dt$lambda_mean,dt$loc_id,dt$Binomial, dt$Log_Body_Mass_g_scale,dt$Class, dt$Protected_status)
  df_sel2<-subset(dt, select=c(sel_clim,sel_lu))
  df_sel<-cbind(df_sel, df_sel2)

  colnames(df_sel)<-c("lambda_mean","loc_id", "Binomial", "body_mass_scale", "Class","Protected_status","mean_slope_scale","change_rate_scale")
  
  df_sel<-df_sel[df_sel$Class == class,]

  df_sel$lambda_mean<-as.numeric(df_sel$lambda_mean)
  df_sel$loc_id<-as.numeric(df_sel$loc_id)
  df_sel$body_mass_scale<-as.numeric(df_sel$body_mass_scale)
  
  m0<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+change_rate_scale:mean_slope_scale+body_mass_scale+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m0f<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+change_rate_scale:mean_slope_scale+body_mass_scale+Protected_status+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m0a<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+body_mass_scale+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m0af<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+body_mass_scale+Protected_status+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m0b<-lmer(lambda_mean ~ change_rate_scale+body_mass_scale+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m0bf<-lmer(lambda_mean ~ change_rate_scale+body_mass_scale+Protected_status+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m0c<-lmer(lambda_mean ~ mean_slope_scale+body_mass_scale+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m0cf<-lmer(lambda_mean ~ mean_slope_scale+body_mass_scale+Protected_status+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m0d<-lmer(lambda_mean ~ body_mass_scale+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m0df<-lmer(lambda_mean ~ body_mass_scale+Protected_status+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m1<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+change_rate_scale:mean_slope_scale+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m1f<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+change_rate_scale:mean_slope_scale+Protected_status+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m1a<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m1af<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+Protected_status+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m1b<-lmer(lambda_mean ~ change_rate_scale+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m1bf<-lmer(lambda_mean ~ change_rate_scale+Protected_status+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)  
  m1c<-lmer(lambda_mean ~ mean_slope_scale+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  m1cf<-lmer(lambda_mean ~ mean_slope_scale+Protected_status+(1|Binomial)+(1|loc_id),data=df_sel, REML=F)
  mnull<-lmer(lambda_mean ~ 1+(1|Binomial)+(1|loc_id),data=df_sel, REML=F) 
 
  mmsAICc <- model.sel(m0,m0f,m0a,m0af,m0b,m0bf,m0c,m0cf,m0d,m0df,m1,m1f,m1a,m1af,m1b,m1bf,m1c,m1cf,mnull)

  mmsAICc$model<-rownames(mmsAICc)
  mmsAICc<-data.frame(mmsAICc)
  mmsAICc
  
  AIC(m0,m0f,m0a,m0af,m0b,m0bf,m0c,m0cf,m0d,m0df,m1,m1f,m1a,m1af,m1b,m1bf,m1c,m1cf,mnull)

  mmodels_list<-list(m0,m0f,m0a,m0af,m0b,m0bf,m0c,m0cf,m0d,m0df,m1,m1f,m1a,m1af,m1b,m1bf,m1c,m1cf,mnull)
  mmodelsR<-lapply(mmodels_list,rsquared.glmm)
  mmodelsRsq <- matrix(unlist(mmodelsR), ncol=6, byrow=T)
  rownames(mmodelsRsq)<-c("m0","m0f","m0a","m0af","m0b","m0bf","m0c","m0cf","m0d","m0df","m1","m1f","m1a","m1af","m1b","m1bf","m1c","m1cf","mnull")
  ...
  mmav<-model.avg(mmodels_list, subset = cumsum(weight) <= 0.95)
  #mmav<-model.avg(mmodels_list, subset = delta <= 6)
  
  msmav<-summary(mmav)
  
  mcoef_av<-msmav$coefmat.subset[1:6,"Estimate"]
  mcoef_df<-data.frame(mcoef_av)
  mcoef_df$lowCI<-confint(mmav)[1:6,1]
  mcoef_df$highCI<-confint(mmav)[1:6,2]
  mcoef_df
  ...
  mcoef_pcnt$class<-class
  mcoef_pcnt$lu<-sel_lu
  mcoef_pcnt$clim<-sel_clim
