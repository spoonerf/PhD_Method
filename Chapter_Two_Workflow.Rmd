...
#Europe
```{r}
df_eu<-subset(df, !is.na(lambda_mean)& !is.na(EOBS_Estimate) & !is.na(CRUTEM_Estimate) & !is.na(HILDA_change) & !is.na(HYDE_change)& !is.na(LUH_change)& !is.na(Binomial) & length_time > 5 )
...
```{r}
df_glb<-subset(df, !is.na(lambda_mean) & !is.na(CRUTEM_Estimate) & !is.na(HYDE_change)& !is.na(LUH_change)& !is.na(Binomial) & length_time > 5 )
...
for (i in 1:nrow(loop_grid)){
  ...
  df_eu_sel<-cbind(dt_eu$lambda_mean,dt_eu$loc_id,dt_eu$Binomial, dt_eu$Log_Body_Mass_g_scale,subset(dt_eu, select=c(sel_clim,sel_lu)))
  colnames(df_eu_sel)<-c("lambda_mean","loc_id", "Binomial", "body_mass_scale", "mean_slope_scale","change_rate_scale")

  mm0<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+change_rate_scale:mean_slope_scale+body_mass_scale+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
  mm0a<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+body_mass_scale+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
  mm0b<-lmer(lambda_mean ~ change_rate_scale+body_mass_scale+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
  mm0c<-lmer(lambda_mean ~ mean_slope_scale+body_mass_scale+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
  mm0d<-lmer(lambda_mean ~ body_mass_scale+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
  mm1<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+change_rate_scale:mean_slope_scale+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
  mm1a<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
  mm1b<-lmer(lambda_mean ~ change_rate_scale+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
  mm1c<-lmer(lambda_mean ~ mean_slope_scale+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
  mmnull<-lmer(lambda_mean ~ 1+(1|Binomial)+(1|loc_id),data=df_eu_sel, REML=F)
