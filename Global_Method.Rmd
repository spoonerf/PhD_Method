...
plot(lu_id$Year, lu_id$Crop, type="l", col="Orange", ylim=c(0,1), ylab="Percentage Cover", xlab="Year")
lines(lu_id$Year, lu_id$Primary, col="Green")
lines(lu_id$Year, lu_id$Pasture, col="Brown")
lines(lu_id$Year, lu_id$Secondary, col="Dark Green")
lines(lu_id$Year, lu_id$Urban, col="Grey")
lines(lu_id$Year, lu_id$Other, col="black")
...
body<-read.csv("bird_and_mammal_traits.csv")
...
df<-merge(df, body[,c(3:5)], by="ID")     #41 pops bodysizes missing for birds

nrow(df)

df2<-subset(df, !is.na(Estimate)&r_sq >= 0.5  & !is.na(LUC_dist)&length_time >=5 & System!="Marine" &Specific_location == 1 & Class=="Mammalia")
...
plot(AIC_m1 - AIC_mnull, main="Difference in AIC between Interacting and Null Model - Mammals", ylab="Delta AIC", xlab="Model Run", )
abline(h=0, col="red", lty =2)
...
colnames(AIC_df)<-c("LUC*MTC", "LUC+MTC", "LUC", "MTC", "Null")
AIC_df<-melt(AIC_df)
colnames(AIC_df)<-c("Model", "AIC")

p<-ggplot(AIC_df, aes(x=factor(Model), y=AIC, fill=factor(Model)))
p+geom_violin(trim=FALSE) +
xlab("Model")+
scale_fill_poke(pokemon = 'Vileplume', spread = 5)
