...
doForest = function(sp_name) {
  ...
  if (sum(!is.na(spidt)) > 1) {
    ...
    luyr<-subset(lu_id, Year>=min(Yr) & Year<= max(Yr))[,c(4:17)]   #6:11 for all categories 12:13 for nat vs anth
    luyr<-luyr[order(luyr$Year),]
    pforest<-luyr$Primary_Forest+luyr
    pfordiff<-mean(diff(pforest))
    psforest<-luyr$Primary_Forest+luyr$Secondary_Forest+luyr
...
df2$Forest_Loss<-(0-df2$Forest_Change)
...
parm_df<-sp_dups_df[,c("ID","Estimate", "Forest_Loss")]  ##ID, land use, and climate  use "LUC_dist" or "Nat_loss" for purely annual change in summed primary, secondary and other 
