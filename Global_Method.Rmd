...
writeRaster(layer_cr, paste(layer, "_1940.tif", sep=""), overwrite=TRUE)
print(i)
...
land_use_layers_tif = c("primf_1940.tif", "primn_1940.tif", "secdf_1940.tif", "secdn_1940.tif","pastr_1940.tif","range_1940.tif","urban_1940.tif","c3ann_1940.tif","c3per_1940.tif","c4ann_1940.tif","c4per_1940.tif","c3nfx_1940.tif")
land_use_result_names = c("primf_r.csv", "primn_r.csv","secdf_r.csv", "secdn_r.csv","pastr_r.csv","range_r.csv","urban_r.csv","c3ann_r.csv","c3per_r.csv","c4ann_r.csv","c4per_r.csv","c3nfx_r.csv")
...
ecoreg <- readOGR(dsn = "C:/Users/Fiona/Desktop/terr-ecoregions-TNC", layer = "tnc_terr_ecoregions")

my_env <- set_env()

get_usage(alg= "grass:v.sample", qgis_env=my_env, intern=TRUE)

params <- get_args_man(alg = "grass:v.sample", 
                       qgis_env = my_env)
...
luc<-read.csv("LUC_average_mean_euc_dist_all_cat_nat_025_moreLPI.csv")
#LPI<-read.csv("LPI_populations_IP_fishedit_20140310_nonconf.csv")
LPI<-read.csv("LPI_pops_20160523_edited.csv")
...
#df2<-subset(df, !is.na(Estimate) & r_sq >= 0.4999999  & !is.na(Nat_change)&length_time >=5 & System!="Marine" &Specific_location == 1 & !is.na(Bodymass) &Class=="Mammalia" ) #& WWF_REALM2!="Palearctic")# & Migratory!=0)    #subset of r_sq >=0.5 not including a population where the value is 0.5 because of a rounding error - population 5458

df2<-subset(df, !is.na(Estimate) & r_sq >= 0.4999999  & !is.na(LUC_dist) &length_time >=5 & System!="Marine" &Specific_location == 1 & !is.na(Bodymass) &Class=="Aves" ) #more populations with this?
...
```{r}
library(ggplot2)
p<-ggplot(df2, aes(x=factor(WWF_REALM2), y=LUC_dist, colour=lambda_mean))
...
p<-ggplot(df2, aes(x=factor(WWF_REALM2), y=LUC_dist, colour=Population_change))
p+geom_jitter(width=0.5, size=2.5, alpha=0.6)+
  labs(x="Biogeographic Realm", y="Annual Change in Land Use")
