...
LPI_EU<-subset(LPI_EU, points_per_pop1950_2005 >=2)
...
write.csv(all_df, "Global_Population_Trends_Rsq_Lambda_07_10_16.csv") ##updated LPI only til 2005
```
...
doMean = function(sp_name) {
  ...
  Mean<-year_temp$min_mean[(min(which(!is.na(spidt)))-lag):max(which(!is.na(spidt)))]
  # 
...
#write.csv(mean_df, "All_LPI_All_Years_Nobuff_1931_moreLPI_min_mean_temp.csv")#annual minimum mean temp
```
...
land_use_layers_tif = c("primf_1940a.tif", "primn_1940a.tif", "secdf_1940a.tif", "secdn_1940a.tif","pastr_1940a.tif","range_1940a.tif","urban_1940a.tif","c3ann_1940a.tif","c3per_1940a.tif","c4ann_1940a.tif","c4per_1940a.tif","c3nfx_1940a.tif")
land_use_result_names = c("primf_ru.csv", "primn_ru.csv","secdf_ru.csv", "secdn_ru.csv","pastr_ru.csv","range_ru.csv","urban_ru.csv","c3ann_ru.csv","c3per_ru.csv","c4ann_ru.csv","c4per_ru.csv","c3nfx_ru.csv")
...
primf2 <- read.csv("primf_ru.csv")#previously without u
primn2<-read.csv("primn_ru.csv")
secdf2<-read.csv("secdf_ru.csv")
secdn2<-read.csv("secdn_ru.csv")
pastr2<-read.csv("pastr_ru.csv")
range2<-read.csv("range_ru.csv")
urban2<-read.csv("urban_ru.csv")
c3ann2<-read.csv("c3ann_ru.csv")
c3per2<-read.csv("c3per_ru.csv")
c4ann2<-read.csv("c4ann_ru.csv")
c4per2<-read.csv("c4per_ru.csv")
c3nfx2<-read.csv("c3nfx_ru.csv")
#water2<-read.csv("water_r.csv")
  
land_use_nat<-data.frame(primf2$lon, primf2$lat, primf2$datesr, primf2$value, primn2$value,secdf2$value, secdn2$value, pastr2$value, range2$value, urban2$value, c3ann2$value, c3per2$value, c4ann2$value, c4per2$value, c3nfx2$value)

colnames(land_use_nat)<-c("Longitude", "Latitude", "Year", "Primary_Forest", "Primary_Non_Forest", 
                          "Secondary_Forest", "Secondary_Non_Forest", "Pasture", "Rangeland", "Urban", "C3ann", "C3per", "C4ann", "C4per", "C3nfx") 

land_use_nat$total<-land_use_nat$Primary_Forest+land_use_nat$Primary_Non_Forest+land_use_nat$Secondary_Forest+
  land_use_nat$Secondary_Non_Forest+ land_use_nat$Pasture+land_use_nat$Rangeland+land_use_nat$Urban+land_use_nat$C3ann+land_use_nat$C3per+land_use_nat$C4ann+land_use_nat$C4per+land_use_nat$C3nfx

land_use_nat$crop<-land_use_nat$C3ann+land_use_nat$C3per+land_use_nat$C4ann+land_use_nat$C4per+land_use_nat$C3nfx
dupe<-ddply(land_use_nat,.(Longitude, Latitude),nrow)

land_use_nat$forest<-land_use_nat$Primary_Forest+land_use_nat$Secondary_Forest
nrow(land_use_nat)

LPI_ID<-LPIsp[,c("ID", "Longitude", "Latitude")]
nrow(LPI_ID)

LPILU<-merge(LPI_ID,land_use_nat, by=c("Longitude", "Latitude"), all=F)
nrow(LPILU)
...
#write.csv(LPILU, "LUH2_Land_Use_All_cats_moreLPI_16_09_13.csv")
write.csv(LPILU, "LUH2_Land_Use_All_cats_moreLPI_16_09_15.csv")
```
...
```{r}
library(plyr)
dupes<-ddply(LPILU,.(ID,Year),nrow)
hist(dupes$V1)
LPILU2<-unique(LPILU)
nrow(LPILU)
nrow(LPILU2)

dup<-ddply(primf2,. (lat,lon),nrow)
 
write.csv(LPILU2, "LUH2_Land_Use_All_cats_moreLPI_16_09_19.csv") #uniqued and with forest
```

```{r}
library(taRifx)

#LPILU<-read.csv("LUH2_Land_Use_All_cats_moreLPI_16_09_13.csv")
...
doDist = function(sp_name) {
  ...
  if (sum(!is.na(spidt)) > 1) {
    ...
    luyr<-subset(lu_id, Year>=min(Yr) & Year<= max(Yr))[,c(4:17)]   #6:11 for all categories 12:13 for nat vs anth
    luyr<-luyr[order(luyr$Year),]
    nat<-luyr$Primary_Forest+luyr$Primary_Non_Forest+luyr$Secondary_Forest+luyr$Secondary_Non_Forest
    natdiff<-mean(diff(nat))
    
    lum<-as.matrix(luyr[,c(2:13)], nrow=length(Yr))
...
#write.csv(mean_df, "LUC_average_mean_euc_dist_all_cat_nat_025_moreLPI_16_09_13.csv")
write.csv(mean_df, "LUC_average_mean_euc_dist_all_cat_nat_025_moreLPI_16_09_15.csv")
...
LPILU<-read.csv("LUH2_Land_Use_All_cats_moreLPI_16_09_19.csv")

doForest = function(sp_name) {
  spid2 = subset(LPIsp, ID == sp_name)   #subsetting the population data by each population
  spid = spid2[,65:130]                     #subsetting only the dates
  colnames(spid)<-1950:2015              #renaming the date column names as R doesn't like numbered column names
  lu_id=subset(LPILU, ID == sp_name)  #subsetting the climate data by each population
  ...
  if (sum(!is.na(spidt)) > 1) {
    ...
    luyr<-subset(lu_id, Year>=min(Yr) & Year<= max(Yr))[,c(4:17)]   #6:11 for all categories 12:13 for nat vs anth
    luyr<-luyr[order(luyr$Year),]
    forest<-luyr$Primary_Forest+luyr$Secondary_Forest
    fordiff<-mean(diff(forest))
    
  } else{
    fordiff<-NA
  }
#  euc_df<-data.frame(id,euc)
euc_df<-data.frame(id,fordiff)
 # euc_df<-data.frame(id, natdiff)
  print(euc_df)  
  return(euc_df)
}

all_df_list <- lapply(unique(LPIsp$ID), doForest)

all_matrix <- matrix(unlist(all_df_list), ncol=2, byrow=TRUE)
mean_df <- data.frame(all_matrix)
colnames(mean_df) <- c("ID", "Forest_Change")

#write.csv(mean_df, "Forest_mean_change_025_moreLPI_16_09_19.csv")
```
...
doForest = function(sp_name) {
  ...
  if (sum(!is.na(spidt)) > 1) {
    ...
    luyr<-subset(lu_id, Year>=min(Yr) & Year<= max(Yr))[,c(4:17)]   #6:11 for all categories 12:13 for nat vs anth
    luyr<-luyr[order(luyr$Year),]
    pforest<-luyr$Primary_Forest
    pfordiff<-mean(diff(pforest))
    psforest<-luyr$Primary_Forest+luyr$Secondary_Forest
    psfordiff<-mean(diff(psforest))
...
#  euc_df<-data.frame(id,euc)
euc_df<-data.frame(id,pfordiff,psfordiff)
 # euc_df<-data.frame(id, natdiff)
  print(euc_df)  
  return(euc_df)
}
...
all_matrix <- matrix(unlist(all_df_list), ncol=3, byrow=TRUE)
mean_df <- data.frame(all_matrix)
colnames(mean_df) <- c("ID", "Prim_Forest_Change", "Prim_Secnd_Forest_Change")

#write.csv(mean_df, "Prim_Forest_mean_change_025_moreLPI_16_09_19.csv")
#write.csv(mean_df, "Prim_Secnd_Forest_mean_change_025_moreLPI_16_09_19.csv")
...
temp<-read.csv("All_LPI_All_Years_Nobuff_1931_moreLPI_end2005.csv")#to go with hyde
# temp<-read.csv("All_LPI_All_Years_Nobuff_1931_moreLPI_min_mean_temp.csv")#annual minimum mean temp
...
```{r}
df2<-subset(dfd, !is.na(Estimate) & r_sq >= 0.4999999  & !is.na(LUC_dist)&length_time >=5 & System!="Marine" &Specific_location == 1 & !is.na(Bodymass)& !is.na(both_change)&Class=="Aves"&((Primary_threat!="Disease"& Primary_threat!="Exploitation"&Primary_threat!="Invasive spp/genes"&Primary_threat!="Pollution") & (Secondary_threat!="Disease"& Secondary_threat!="Exploitation"&Secondary_threat!="Invasive spp/genes"&Secondary_threat!="Pollution") & (Tertiary_threat!="Disease"& Tertiary_threat!="Exploitation"&Tertiary_threat!="Invasive spp/genes"&Tertiary_threat!="Pollution")))

#&Forest==1&((Primary_threat =="Habitat degradation/change"| Primary_threat=="Habitat loss"|Primary_threat=="Climate change") | (Secondary_threat =="Habitat degradation/change"| Secondary_threat=="Habitat loss"|Secondary_threat=="Climate change") | (Tertiary_threat == "Habitat degradation/change"| Tertiary_threat=="Habitat loss"|Tertiary_threat=="Climate change")))


            
#            &((Primary_threat!="Disease"& Primary_threat!="Exploitation"&Primary_threat!="Invasive spp/genes"&Primary_threat!="Pollution") & (Secondary_threat!="Disease"& Secondary_threat!="Exploitation"&Secondary_threat!="Invasive spp/genes"&Secondary_threat!="Pollution") & (Tertiary_threat!="Disease"& Tertiary_threat!="Exploitation"&Tertiary_threat!="Invasive spp/genes"&Tertiary_threat!="Pollution")))

            
            #&((Primary_threat =="Habitat degradation/change"| Primary_threat=="Habitat loss"|Primary_threat=="Climate change") | (Secondary_threat =="Habitat degradation/change"| Secondary_threat=="Habitat loss"|Secondary_threat=="Climate change") | (Tertiary_threat == "Habitat degradation/change"| Tertiary_threat=="Habitat loss"|Tertiary_threat=="Climate change")))
...
#& (Primary_threat != "Habitat degradation/change" &Primary_threat != "Habitat loss" & Primary_threat != "Climate change" & Primary_threat !="NULL") & (Secondary_threat != "Habitat degradation/change" &Secondary_threat != "Habitat loss" & Secondary_threat != "Climate change" & Secondary_threat != "NULL")& (Tertiary_threat != "Habitat degradation/change" & Tertiary_threat != "Habitat loss" & Tertiary_threat != "Climate change" & Tertiary_threat != "NULL" ))
...
#df2$Prim_Forest_Loss<-(0-df2$Prim_Forest_Change)
#df2$Prim_Secnd_Forest_Loss<-(0-df2$Prim_Secnd_Forest_Change)
...
parm_df<-sp_dups_df[,c("ID","Estimate", "both_change", "Bodymass")]  ##ID, land use, and climate  use "LUC_dist" or "Nat_loss" for purely annual change in summed primary, secondary and other 
...
for (i in 1:R) {
 ...
 m0<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+change_rate_scale:mean_slope_scale+Bodymass_scale+(1|Binomial),data=sp_dups_df2, REML=F)

 m0a<-lmer(lambda_mean ~ change_rate_scale+mean_slope_scale+Bodymass_scale+(1|Binomial),data=sp_dups_df2, REML=F)

 m0b<-lmer(lambda_mean ~ change_rate_scale+Bodymass_scale+(1|Binomial),data=sp_dups_df2, REML=F)

 m0c<-lmer(lambda_mean ~ mean_slope_scale+Bodymass_scale+(1|Binomial),data=sp_dups_df2, REML=F)

 m0d<-lmer(lambda_mean ~ Bodymass_scale+(1|Binomial),data=sp_dups_df2, REML=F)
  ...
  BM_i[i]<-var_imp$importance["Bodymass_scale"]
  ...
  BM_av[i]<-var_imp$coefmat.subset["Bodymass_scale", "Estimate"]
...
coef_pcnt<-((10^conf_av) - 1)*100
plotCI(1:4, coef_pcnt[2,], (coef_pcnt[3,]-coef_pcnt[2,]), (coef_pcnt[2,]-coef_pcnt[1,]), ylab="Annual Population Change", xlab="" ,xaxt = "n", main="", lwd=1, ylim=c(min(coef_pcnt[1,]*1.1), max(coef_pcnt[3,]*1.1)))
axis(1, at=1:4, labels=colnames(conf_av), las=2)
abline(h=0, col="red", lty =2)
...
boxplot(marg_df, main="R Squared Birds and Mammals", ylab= "Marginal R Squared (%)")
```
...
xlab("Model")+
scale_fill_poke(pokemon = 'Vileplume', spread = length(unique(AIC_del2$Model)))
