...
<<<<<<< HEAD
df2<-subset(df, !is.na(Estimate) & r_sq >= 0.4999999  & !is.na(Nat_change)&length_time >=5 & System!="Marine" &Specific_location == 1 & !is.na(Bodymass) &Class=="Aves")#& WWF_REALM2=="Neotropic")# & Migratory!=0)    #subset of r_sq >=0.5 not including a population where the value is 0.5 because of a rounding error - population 5458
=======
df2<-subset(df, !is.na(Estimate) & r_sq >= 0.4999999  & !is.na(Nat_change)&length_time >=5 & System!="Marine" &Specific_location == 1 & !is.na(Bodymass) ) #& WWF_REALM2!="Palearctic")# & Migratory!=0)    #subset of r_sq >=0.5 not including a population where the value is 0.5 because of a rounding error - population 5458
...
>>>>>>> ff9052cec0ff904f47e2b8ed2ac47fde63d3a049
...
```{r}
library(ggplot2)
p<-ggplot(df2, aes(x=factor(WWF_REALM2), y=Nat_change*100, colour=lambda_mean))
p+geom_jitter(width=0.5, size=2.5, alpha=0.6)+
  ...
  labs(x="Biogeographic Realm", y="Annual Change in Natural Cover (%)")


df2$Population_change<-numeric(length(df2$ID))

df2[df2$lambda_mean >= log10(1.02),]$Population_change<- "Increasing"
df2[df2$lambda_mean <= log10(0.98),]$Population_change<- "Decreasing"
df2[df2$lambda_mean >= log10(0.98) & df2$lambda_mean <= log10(1.02),]$Population_change<- "Stable"

p<-ggplot(df2, aes(x=factor(WWF_REALM2), y=Nat_change*100, colour=Population_change))
p+geom_jitter(width=0.5, size=2.5, alpha=0.6)+
  labs(x="Biogeographic Realm", y="Annual Change in Natural Cover (%)")
