...
n<-3#number of cores to use - not sure how many I can go up to - 6 at UCL
cl<-makeCluster(n)
...
df2<-subset(df, !is.na(Estimate) & r_sq >= 0.4999999  & !is.na(Nat_change)&length_time >=5 & System!="Marine" &Specific_location == 1 & !is.na(Bodymass) &Class=="Aves")    #subset of r_sq >=0.5 not including a population where the value is 0.5 because of a rounding error - population 5458
...
```{r}
test<-read.csv("birds_for_lme.csv")
test$ID[!test$ID %in% df2$ID]


subset(df, ID == 5458)$r_sq >=0.5
...
sp_df_scale[is.na(sp_df_scale$lambda_mean),]$lambda_mean<-0  #####is this a problem - changing NAs for 0 - when the population is stationary across years the lambda mean is calculated
...
```{r}
AIC_df<-data.frame(cbind(AIC_m0,AIC_m0a,AIC_m0b,AIC_m0c,AIC_m0d,AIC_m1,AIC_m1a, AIC_m1b, AIC_m1c, AIC_mnull))

AIC_del<-AIC_df[,c(1:length(AIC_df)-1)] - AIC_df$AIC_mnull
...
#marg r squared
marg_df<- data.frame(cbind(marg_Rsq_m0,marg_Rsq_m0a,marg_Rsq_m0b,marg_Rsq_m0c,marg_Rsq_m0d,marg_Rsq_m1,marg_Rsq_m1a,marg_Rsq_m1b,marg_Rsq_m1c))

colnames(marg_df)<-c("m0","m0a","m0b","m0c","m0d","m1","m1a","m1b","m1c")

boxplot(marg_df, main="Marginal R squared value of model")
```

```{r}
#cond r squared
cond_df<- data.frame(cbind(cond_Rsq_m0,cond_Rsq_m0a,cond_Rsq_m0b,cond_Rsq_m0c,cond_Rsq_m0d,cond_Rsq_m1,cond_Rsq_m1a,cond_Rsq_m1b,cond_Rsq_m1c,cond_Rsq_mnull))
colnames(cond_df)<-c("m0","m0a","m0b","m0c","m0d","m1","m1a","m1b","m1c", "mnull")

boxplot(cond_df, main="Conditional R squared value of model")
```

```{r}
#model weight in averaged model
weight_df<-data.frame(cbind(m0_w,m0a_w,m0b_w,m0c_w,m0d_w,m1_w,m1a_w,m1b_w,m1c_w,mnull_w))

colnames(weight_df)<-c("m0","m0a","m0b","m0c","m0d","m1","m1a","m1b","m1c", "mnull")

boxplot(weight_df, main="Weight of model in average model across bootstraps")
```
...
colnames(vi_df)<-c("MTC", "LUC", "BM", "LUC_MTC")
