...
land_use_layers = list(primf, primn, secdf, secdn)
land_use_result_names = list("primf_r.csv", "primn_r.csv","secdf_r.csv", "secdn_r.csv")
...
for (i in 1:length(land_use_layers)) {
  layer = land_use_layers[[i]]
...
land_use_nat$total<-land_use_nat$Primary_Forest+land_use_nat$Primary_Non_Forest+land_use_nat$Secondary_Forest+
  land_use_nat$Secondary_Non_Forest
...
doDist = function(sp_name) {
  ...
  if (sum(!is.na(spidt)) > 1) {
    ...
    luyr<-as.matrix(subset(lu_id, Year>=min(Yr) & Year<= max(Yr))[,c("Year","total")])   #6:11 for all categories 12:13 for nat vs anth
    luyr<-luyr[order(luyr[,1]),]
    natdiff<-mean(diff(luyr[,2]))
    
    # lum<-matrix(luyr[,2], ncol=1)
