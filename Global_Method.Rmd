...
df2<-subset(df, !is.na(Estimate)&r_sq >= 0.5  & !is.na(Nat_change)&length_time >=5 & System!="Marine" &Specific_location == 1 & (Class=="Aves") & !is.na(Bodymass) )
...
for (i in 1:R) {
 ...
 colnames(dt2)[2]<-"ID"
  sp_dups_df2<-sp_df_scale[sp_df_scale$ID %in% dt$ID,]    #sp_df_scale   #should be dt2
...
AIC_del<-melt(AIC_del)
colnames(AIC_del)<-c("Model", "AIC")

p<-ggplot(AIC_del, aes(x=factor(Model), y=AIC, fill=factor(Model)))
