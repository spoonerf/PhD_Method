...
###difference between luc and luc2 for same populations

Realm<-read.csv("Ecoregions_coordinates.csv")
Realm<-Realm[,c("ID", "WWF_REALM2")]
colnames(Realm)<-c("ID", "Realm")
#Realm2<-merge(LPI, Realm, by="ID")
...
#
df2<-subset(df, !is.na(Estimate) & r_sq >= 0.4999999  & !is.na(Nat_change)&length_time >=5 & System!="Marine" &Specific_location == 1 & !is.na(Bodymass) &Class=="Aves" ) #& WWF_REALM2!="Palearctic")# & Migratory!=0)    #subset of r_sq >=0.5 not including a population where the value is 0.5 because of a rounding error - population 5458

df3<-subset(df, !is.na(Estimate) & r_sq >= 0.4999999  & !is.na(LUC_dist) &length_time >=5 & System!="Marine" &Specific_location == 1 & !is.na(Bodymass) &Class=="Aves" ) #more populations with this?
