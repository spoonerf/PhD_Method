...
var_grid[,1]<-qunif(var_grid[,1],0, 0.5) #SD
var_grid[,2]<-qunif(var_grid[,2],0, 0.5) #SDD
var_grid[,3]<-qunif(var_grid[,3],0, 0.5) #LDD
var_grid[,4]<-qexp(var_grid[,4], qexp_rate)#dens
var_grid[,5]<-qunif(var_grid[,5],carry_k_l, carry_k_h)#carrying capacity
var_grid[,6]<-qunif(var_grid[,6],med_disp_l, med_disp_h)#med disp
var_grid[,7]<-qunif(var_grid[,7],max_disp_l, max_disp_h)#max disp


var_grid

colnames(var_grid)<-c("SD","SDD" ,"LDD", "Density", "Carry_K", "Med_Disp", "Max_Disp")
...
#write.csv(targetBg, paste(species_directory,"background_data_et_ungulates.csv", sep="/"))
```

```{r}
bg_pse<-read.csv(paste(species_directory,"background_data_et_ungulates.csv", sep="/"))

bg_pse<-bg_pse[,-1]

k<-4

targetBg_k<-kfold(targetBg, k)
...
sdm<-raster(paste(sdm_folder, "/hyde_pres_abs_sss_weighted_ensemble_sdm_", years[1],".tif", sep=""))
e2<-extent(sdm)
...
opt_patch_spin_up[,4:length(spin_years)+3]<-1
...
```{r demoniche function, message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE}
reps<-1
