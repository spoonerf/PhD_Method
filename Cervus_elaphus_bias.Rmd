...
sdm_lpi<-extract(sdm_stack, xy_lpi, buffer = 50000, fun = mean)
...
sdm_lpi_both<-join(melt_lpi, sdm_lpi_melt)

m1<-rq(Abundance~HSI, tau = 0.97, data = sdm_lpi_both)
...
plot( sdm_lpi_both$HSI, log10(sdm_lpi_both$Abundance+1), ylab = "Log10(Abundance + 1)", xlab = "Habitat Suitability", main = "Red Deer")
abline(rq(log10(sdm_lpi_both$Abundance+1) ~ sdm_lpi_both$HSI, tau = 0.95), col = "blue")
abline(rq(log10(sdm_lpi_both$Abundance+1) ~ sdm_lpi_both$HSI, tau = 0.5), col = "red")



sdm_lpi_both_cc<-sdm_lpi_both[complete.cases(sdm_lpi_both),]
fitOs.lqmm <- lqmm(log10(Abundance+1) ~ HSI, random = ~ ID, group = ID ,tau = 0.95, data = sdm_lpi_both_cc)
fitOi.lqmm <- lqmm(log10(Abundance+1) ~ HSI, random = ~ 1, group = ID ,tau = 0.5, data = sdm_lpi_both_cc)
...
m1m<-lmer(log10(Abundance+1)~ HSI +(1|ID), data = sdm_lpi_both_cc)

plot( sdm_lpi_both_cc$HSI,fitted.values(m1m))
```
...
gam_lpi_r_nl<-apply(popsm,  1, gam_lpi)
gam_r_nl<-do.call( "rbind", gam_lpi_r_nl)

gam_r_nl<-gam_r_nl[gam_r_nl$Year <=2005,]

fill_nl<-data.frame(rep(pops$ID, each=length(1950:2005)), 1950:2005)
colnames(fill_nl)<-c("ID", "Year")

all_year_ab_nl<-join(fill_nl, gam_r_nl, type="right")

all_year_ab_nl$ID<-as.numeric(as.character(all_year_ab_nl$ID))

ggplot(data =all_year_ab_nl, aes(x = Year, y= Abundance, group=ID))+
  geom_line()+
  facet_grid(.~ID)


ggplot(data =all_year_ab, aes(x = Year, y= Lambdas, group=ID))+
  geom_line()+
  facet_grid(.~ID)
...
sdm_ab_df<-join(sdm_lpi_melt, all_year_ab_nl)
sdm_ab_df$ID<-as.factor(sdm_ab_df$ID)

ggplot(sdm_ab_df, aes(x = HSI, y = log10(Abundance), group = ID, colour = ID))+
  geom_point()+
  facet_grid(.~ID)

plot(sdm_ab_df$HSI, sdm_ab_df$Abundance)

```

```{r}
sdm_lmb_df<-join(sdm_lpi_melt, all_year_ab)
sdm_lmb_df$ID<-as.factor(sdm_lpi_melt$ID)

ggplot(sdm_lmb_df, aes(x = HSI, y = Lambdas, group = ID, colour = ID))+
  geom_point()

plot(sdm_lmb_df$HSI, sdm_lmb_df$Lambdas)
...
ccf_get<-function(x){
  ...
  obs_x<-all_year_ab_nl[all_year_ab_nl$ID == unique(all_year_ab_nl$ID)[x],]
  #obs_x<-obs_x[obs_x$Year >= 1951,]
  sdm_x<-sdm_lpi_melt[sdm_lpi_melt$ID == unique(sdm_lpi_melt$ID)[x],]
  ...
  sdm_ccf<-ccf(sdm_x$HSI[start_sdm:end_sdm], obs_x$Abundance[start_obs:end_obs], type="correlation")
  ...
  ccf_out<-data.frame(unique(sdm_x$ID), lag_n3_sdm, lag_n2_sdm,  lag_n1_sdm,  lag_0_sdm, lag_1_sdm, lag_2_sdm, lag_3_sdm )
  colnames(ccf_out)<-c("ID","sdm_n3","sdm_n2", "sdm_n1",  "sdm_0", "sdm_1", "sdm_2", "sdm_3")
...
#correlation between habitat suitability and population growth rate

ccf_scores<-lapply(1:length(unique(sdm_lmb_df$ID)), ccf_get)
...
# colnames(ccf_df)<-c("ID","ldd","SD","cnd lag -5", "sdm lag -5","cnd lag -4", "sdm lag -4","cnd lag -3", "sdm lag -3","cnd lag -2", "sdm lag -2","cnd lag -1", "sdm lag -1", "cnd lag 0", "sdm lag 0","cnd lag 1", "sdm lag 1","cnd lag 2", "sdm lag 2","cnd lag 3", "sdm lag 3","cnd lag 4", "sdm lag 4","cnd lag 5", "sdm lag 5")
 ...
 colnames(ccf_df)<-c("ID","cnd lag -3", "sdm lag -3","cnd lag -2", "sdm lag -2","cnd lag -1", "sdm lag -1", "cnd lag 0", "sdm lag 0","cnd lag 1", "sdm lag 1","cnd lag 2", "sdm lag 2","cnd lag 3", "sdm lag 3")


ccf_melt<-melt(ccf_df, id.vars = "ID")
ccf_melt$ID<-as.factor(ccf_melt$ID)

```

```{r}
#abundance against HSI

ggplot(ccf_melt, aes(x=variable, y=value, group = variable, colour = ID)) + 
  geom_violin()+
  geom_jitter()
```
...
ccf_get<-function(x){
  ...
  sdm_ccf<-ccf(sdm_x$HSI[start_sdm:end_sdm], obs_x$Lambdas[start_obs:end_obs], type="correlation")
...
colnames(ccf_df)<-c("ID", "sdm lag -3", "sdm lag -2", "sdm lag -1",  "sdm lag 0", "sdm lag 1", "sdm lag 2", "sdm lag 3")
...
sdm_lambda_both<-join(all_year_ab, sdm_lpi_melt)

m1<-rq(Lambdas~HSI, tau = 0.97, data = sdm_lambda_both)
...
plot( sdm_lambda_both$HSI, sdm_lambda_both$Lambdas, main = "Red Deer", ylab="Lambda", xlab = "Habitat Suitability")
abline(rq(sdm_lambda_both$Lambdas ~ sdm_lambda_both$HSI, tau = 0.95), col = "blue")
abline(rq(sdm_lambda_both$Lambdas ~ sdm_lambda_both$HSI, tau = 0.5), col = "red")

sdm_lambda_both_cc<-sdm_lambda_both[complete.cases(sdm_lambda_both),]
 
mlm<-lmer(Lambdas~ HSI +(1|ID), data = sdm_lambda_both_cc)

plot( sdm_lambda_both_cc$HSI,fitted.values(mlm))
```
