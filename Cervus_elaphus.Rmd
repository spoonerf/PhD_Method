...
output: 
  prettydoc::html_pretty:
    ...
    highlight: github
---
###Things that need to be sorted manually:

* Matrices 
* Checking of GBIF data 
* Writing GBIF data
* Writing K fold data
...
#- just doing equal splits for now
...
density_mine<- 1564  #upper value 863
carry_k<-1617
...
pyr<-subset(lpi, Binomial == binomial & Specific_location==1 & Longitude > min_lon & Longitude < max_lon & Latitude > min_lat & Latitude < max_lat)    #record 11470 had wrong longitude - in Russia!
...
#projectRaster(pred_crop[[1]], crs = "+proj=laea +lat_0=52 +lon_0=10 +x_0=4321000 +y_0=3210000 +ellps=GRS80 +units=m +no_defs" )
...
test_val<-extract(sdms[[1]],Populations[,c(2,3)])
pop_na<-which(is.na(test_val))
Populations<-Populations[-(pop_na), ]
...
MatList1<-list(tempMat[[2]][[1]])  #varies depending on number of matrices - need to find a way to code this better - now have five matrices available so need to sort this
AllMat1<-unlist(MatList1)

MatList2<-list(tempMat[[3]][[1]])  #varies depending on number of matrices - need to find a way to code this better - now have five matrices available so need to sort this
AllMat2<-unlist(MatList2)

MatList3<-list(tempMat[[4]][[1]])  #varies depending on number of matrices - need to find a way to code this better - now have five matrices available so need to sort this
AllMat3<-unlist(MatList3)

matrix_bind<-cbind(AllMat1, AllMat2, AllMat3)

matrices<-matrix(matrix_bind, ncol=ncol(matrix_bind))
colnames(matrices)<- c("Reference_matrix", "Matrix_A", "Matrix_B")
...
dir.create(paste(demoniche_folder, "/max_disp_",max_disp,sep=""),showWarnings = TRUE)
...
c_ibex_k_16000 <- demoniche_model_me(modelname = binomial, Niche = TRUE, 
                                     ...
                                     foldername = paste(binomial, "/Demoniche_Output/max_disp_",max_disp,"/",i, sep=""))
}
...
```{r,message=FALSE, warning=FALSE, echo=FALSE}
 ...
 plot_rep<-function(proj_path){
  ...
  load(proj_path)
  id<-gsub(paste(demoniche_folder, "/max_disp_", sep=""), "", proj_path)
