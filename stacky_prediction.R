...
df2<-subset(df, !is.na(Estimate)&r_sq >= 0.5  & !is.na(LUC_dist)&length_time>=5 & System!="Marine" 
            &Specific_location == 1 & (Class =="Aves" | Class == "Mammalia") )
...
for (i in 1:R) {
  ...
  m1<-lmer(lambda_sum ~ change_rate_scale+mean_slope_scale+change_rate_scale:mean_slope_scale+(1|Binomial) + (1|WWF_REALM2),data=sp_dups_df2, REML=F)
  #m1T<-lmer(lambda_sum ~ change_rate_scale+mean_slope_scale+change_rate_scale:mean_slope_scale+(1|Binomial),data=sp_dups_df2)
  
  m1a<-lmer(lambda_sum ~ change_rate_scale+mean_slope_scale+(1|Binomial)+ (1|WWF_REALM2),data=sp_dups_df2, REML=F)
  #m1aT<-lmer(lambda_sum ~ change_rate_scale+mean_slope_scale+(1|Binomial),data=sp_dups_df2)
  
  m1b<-lmer(lambda_sum ~ change_rate_scale+(1|Binomial)+ (1|WWF_REALM2),data=sp_dups_df2, REML=F)
  #m1bT<-lmer(lambda_sum ~ change_rate_scale+(1|Binomial),data=sp_dups_df2)
  
  m1c<-lmer(lambda_sum ~ mean_slope_scale+(1|Binomial)+ (1|WWF_REALM2),data=sp_dups_df2, REML=F)
  #m1cT<-lmer(lambda_sum ~ mean_slope_scale+(1|Binomial),data=sp_dups_df2)
  
  mnull<-lmer(lambda_sum ~ 1+(1|Binomial)+ (1|WWF_REALM2),data=sp_dups_df2, REML=F)
