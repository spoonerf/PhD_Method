...
min_lat<- 37   #one degree more/less than the iucn range extent
max_lat<- 84
min_lon<- -180
max_lon<- 180

e_iucn<-extent(min_lon, max_lon, min_lat, max_lat)
bioclim_layers<-c(5,6,8,11,13)   
...
SD<-c(0.1,0.25, 0.5)
 ...
 kern_seq<-list(c(129.8,742), c(1000,1000))   #first values is median derived from median home range and second value is maximum dispersal distance in km2
# 
density_mine<- 0.00792  
carry_k<-c(0.01,0.1) #per km2
...
sp<-sp[sp@coords[,2] >min_lat & sp@coords[,2] <max_lat,]
...
for (i in 1:k){
  ...
  gm1<-gam(pa~ s(Bio_5_2006_2016_average)+s(Bio_6_2006_2016_average)+ s(Bio_8_2006_2016_average)+ 
...
model<-pa~  Bio_5_2006_2016_average+Bio_6_2006_2016_average+ 
  Bio_8_2006_2016_average+ Bio_11_2006_2016_average+ 
...
gm1<-mgcv:::gam(pa~ s(Bio_5_2006_2016_average)+s(Bio_6_2006_2016_average)+ s(Bio_8_2006_2016_average)+ 
           s(Bio_11_2006_2016_average)+ s(Bio_13_2006_2016_average)+ + s(Hyde_Year_2005), family = binomial(link = "logit"),data=env_all)
...
matrices<-cbind(matrices, matricesA)
