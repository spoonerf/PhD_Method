...
#kern_seq<-list(c(1,1), c(19.775,113.000),c(13.5975, 77.700), c(1000.000,1000.000))
SD<-c(0.1,0.25, 0.5)

med_disp<-seq(13.5975, 19.775, length.out = 5)
#max_disp<-seq(77.7, 113, length.out = 5)

#how to deal with max_dispersal



#kern_seq<-list(c(13.5975, 77.700),c(19.775,113.000))
...
for (i in 1:nlayers(hyde_resample)){
  ...
  writeRaster(hyde_resample[[i]], paste(wd,"/hyde/", names(hyde_resample)[i], ".tif",sep=""))
  print(i)
...
hyde_resample<-stack(paste(wd,"/hyde/Hyde_Year_", years, ".tif", sep = ""))

#bioclim_layers<-c(3,10,12,13,18)
if (Europe_only){
...
# e_nor<-extent(0,120,50,80)
# 
# pred_nor<-crop(pred_nf, e_nor)
plot(pred_crop)
...
envtrain <- extract(pred_crop, train)
...
envtest <- extract(pred_crop, test)
...
env_back<-extract(pred_crop, bg_pse)
...
for (i in 1:k){
  ...
  gm1<-gam(pa~ s(Bio_1_2006_2016_average)+s(Bio_5_2006_2016_average)+ s(Bio_6_2006_2016_average)+
           ...
           s(Bio_19_2006_2016_average) + s(Hyde_Year_2005), family = binomial(link = "logit"),data=envtrain)
...
model<-pa~  Bio_1_2006_2016_average+Bio_5_2006_2016_average+
  ...
  Bio_18_2006_2016_average+ Bio_19_2006_2016_average + Hyde_Year_2005
...
gm1<-gam(pa~ s(Bio_1_2006_2016_average)+s(Bio_5_2006_2016_average)+ s(Bio_6_2006_2016_average)+
           s(Bio_13_2006_2016_average)+ s(Bio_15_2006_2016_average)+ s(Bio_18_2006_2016_average)+
           s(Bio_19_2006_2016_average) +s(Hyde_Year_2005), family = binomial(link = "logit"),data=env_all)
...
#cols <-colorspace::terrain_hcl(10, c=c(65,0), l=c(45,95), power=c(1/3,1.5))


library(RColorBrewer)
cols <-brewer.pal(10, "YlGn")
breakpoints<-seq(0.1,1,by=0.1)
plot(models[[1]],axes=FALSE,legend=TRUE,main =names(models[[1]]),  col=cols, breaks=breakpoints) 
maps:::map('world', xlim=c(e_big[1],e_big[2]), ylim=c(e_big[3],e_big[4]), add=T)

plot(models[[2]],axes=FALSE,legend=TRUE,main =names(models[[2]]),  col=cols, breaks=breakpoints) 
maps:::map('world', xlim=c(e_big[1],e_big[2]), ylim=c(e_big[3],e_big[4]), add=T)

plot(models[[3]],axes=FALSE,legend=TRUE,main =names(models[[3]]),  col=cols, breaks=breakpoints) 
maps:::map('world', xlim=c(e_big[1],e_big[2]), ylim=c(e_big[3],e_big[4]), add=T)
...
plot(wm, main="Ensemble model",axes=FALSE,legend=TRUE, col=cols, breaks=breakpoints)
maps:::map('world', xlim=c(e_big[1],e_big[2]), ylim=c(e_big[3],e_big[4]), add=T)
points(sp)
...
bc <- bioclim(pred_nf, sp)
gm1<-mgcv:::gam(pa~ s(Bio_1_2006_2016_average)+ s(Bio_5_2006_2016_average)+ s(Bio_6_2006_2016_average)+ s(Bio_13_2006_2016_average)+ s(Bio_15_2006_2016_average)+s(Bio_18_2006_2016_average)+ s(Bio_19_2006_2016_average) + s(Hyde_Year_2005), data=env_all)
...
colnames(link_spin1)[1:length(spin_years)]<-paste("weighted_ensemble_sdm_", spin_years, sep="")

spin2<-replicate(length(spin_years),link2[,1])
link_spin2<-cbind(spin2, link2)
colnames(link_spin2)[1:length(spin_years)]<-paste("weighted_ensemble_sdm_", spin_years, sep="")

spin3<-replicate(length(spin_years),link3[,1])
link_spin3<-cbind(spin3, link3)
colnames(link_spin3)[1:length(spin_years)]<-paste("weighted_ensemble_sdm_", spin_years, sep="")
...
for (s in 1:nrow(var_grid)){
  ...
  dir.create(paste(demoniche_folder,"/new_patch_disp_test_",med_disp,"_",SDD,"_",LDD,"_",kern,"_",SD,"_",K,"_",dens,"_",link_id, "/",sep=""),showWarnings = TRUE)
...
c_ibex_k_16000 <- demoniche_model_me(modelname = binomial, Niche = TRUE,
                                     ...
                                     foldername = paste(binomial,"/Demoniche_Output/new_patch_disp_test_",med_disp,"_",SDD,"_",LDD,"_",kern,"_",SD,"_",K,"_",dens,"_",link_id,"/",i, sep = ""))
 }
...
ggplot(all_year_ab, aes(x = year, y = value, group= sp_lpi.ID, col=sp_lpi.ID))+
  geom_line(size =2)+
  xlab("Year")+
  ylab("Ibex abundance")+
...
both_df<-both_df[both_df$max_disp != "1000" & both_df$sp_lpi.ID != 10710,]
