...
opt_patch_spin_up[,4:304]<-1

plot(min(spin_years):max(years),colMeans(na.omit(opt_patch_spin_up)[4:length(colnames(opt_patch_spin_up))]), type="l", ylab="Mean suitability index")
...
for (s in 1:nrow(var_grid)){
  ...
  matrices_var<-matrix(0, ncol = 1, nrow = nrow(matrices), dimnames = list(NULL, "sd"))

start.time <- Sys.time()

max_disp<-as.character(kern_mat[s,2]) #this should be 2?
...
rep_demoniche<-function(i){
 ...
 demoniche_setup_me(modelname = binomial ,Populations = Populations, Nichemap = patch_spin_up,
                matrices = matrices,matrices_var = matrices_var, prob_scenario = prob_scenario,
                  ...
                  density_individuals = 2671,
                  fraction_LDD = 0.25, fraction_SDD = 0.5,
