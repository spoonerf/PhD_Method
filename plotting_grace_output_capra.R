...
#write.csv(df, "capra_ibex_results_all.csv")
#save(df, file = "capra_ibex_results_all.RData")
...
sdm_folder<-paste(wd, "/Legion/snow_capra_bias/", sep="")
...
#checking the lambdas are in the right order - double check because dplyr is acting up
...
#write.csv(rmse_out, "rmse_out_capra_all.csv")
...
#need to get rid of runs that crash out
...
ccf_get<-function(x){
  ...
  if (nrow(cnd_x)>0){
    ...
    cnd_ccf<-ccf(cnd_x$Lambdas[start_cnd:end_cnd], obs_x$Lambdas[start_obs:end_obs], type="correlation", plot = F)
    sdm_ccf<-ccf(sdm_x$HSI_Lambdas[start_sdm:end_sdm], obs_x$Lambdas[start_obs:end_obs], type="correlation", plot = F)
...
#write.table(ccf_melt, "ccf_melt_capra.csv")
# #save(ccf_melt, file = "ccf_melt_ursus2.RData")
ccf_melt<-read.table("ccf_melt_capra.csv")
